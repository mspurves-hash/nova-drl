...
def relevant_snippets(search, event: Dict[str, Any], item_label: str) -> List[str]:
    try:
        snippets = list(search._technician_repair_snippets(event))
    except Exception:
        snippets = []
    ...
    words = normalized_words(item_label)
    if not words:
        return snippets[:4]

    selected = []
    for s in snippets:
        sw = set(normalized_words(s))
        if any(w in sw for w in words):
            selected.append(str(s))
    return selected[:6] if selected else snippets[:3]
...
def render_event(
    ...
    if kind == "part":
        ...
        snippets = relevant_snippets(search, event, label)
        for s in snippets:
            print(f"   Repair evidence: {' '.join(str(s).split())}")
...
def render_header(
    ...
    print("=" * 92)
    print("NOVA DRL EVIDENCE DRILL-DOWN  |  v1.8.0")
    print("=" * 92)
...
def run(args) -> int:
    search, family, gate = build_stack()
    ...
    for i, event in enumerate(page, offset + 1):
        ...
        render_event(kind, display_label(row), event, conn, search, gate)
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Nova DRL v1.8.0 family-scoped evidence drill-down"
    )
