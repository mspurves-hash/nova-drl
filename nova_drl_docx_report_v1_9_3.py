#!/usr/bin/env python3
"""
NOVA DRL Editable DOCX Report v1.9.3
...
Examples:
  python3 nova_drl_docx_report_v1_9_2.py --search "MR-J2S-40A"
  python3 nova_drl_docx_report_v1_9_2.py --search "MR-J2S-40A" --out /tmp/MR-J2S-40A.docx
  python3 nova_drl_docx_report_v1_9_2.py --from-text report.txt --search "MR-J2S-40A" --out sample.docx
"""
...
DEFAULT_REPORT_DIR = "/mnt/drl-reports"
...
def protected_output_path(
    report: Dict[str, Any],
    requested_out: Optional[str] = None,
    force: bool = False,
    ...
    Returns (output_path, protected_existing_path).

    Normal/default workflow:
      - canonical filename if it does not exist;
      - timestamped _NEW_ copy if canonical filename already exists;
      - --force explicitly allows replacing the canonical file.

    Explicit --out workflow:
      - exact requested path if new;
      - refuse to overwrite an existing file unless --force is supplied.
    """
    if requested_out:
        chosen = Path(requested_out)
        if chosen.exists() and not force:
            raise FileExistsError(
                f"Refusing to overwrite existing DOCX: {chosen}\n"
                f"Use a different --out path or add --force only if replacement is intentional."
            )
        return chosen, chosen if chosen.exists() else None

    canonical = default_out(report)
    if force or not canonical.exists():
        return canonical, canonical if canonical.exists() else None

    protected = canonical
    stamp_dt = now or dt.datetime.now().astimezone()
    stamp = stamp_dt.strftime("%Y-%m-%d_%H%M%S")
    stem = canonical.stem
    suffix = canonical.suffix or ".docx"
    candidate = canonical.with_name(f"{stem}_NEW_{stamp}{suffix}")
    counter = 2
    while candidate.exists():
        candidate = canonical.with_name(f"{stem}_NEW_{stamp}_{counter}{suffix}")
        counter += 1
...
def main() -> int:
    ...
    ap.add_argument("--force", action="store_true", help="Allow intentional overwrite of an existing DOCX")
    ...
    try:
        ...
        out, protected = protected_output_path(report, args.out, args.force)
        build_docx(report, out)
    except (FileExistsError, ValueError, RuntimeError, OSError) as exc:
        print(f"ERROR: NOVA DOCX not created: {exc}", file=sys.stderr)
        print("Use a more complete DRL Part # if the search did not resolve one unique equipment family.", file=sys.stderr)
        return 2

    print("# NOVA DRL Editable DOCX Report | v1.9.3")
    ...
    if protected and protected != out:
        print(f"Existing technician DOCX protected: {protected}")
        print(f"New DOCX created instead: {out}")
    elif protected and args.force:
        print(f"Existing DOCX intentionally overwritten with --force: {out}")
    print(f"DOCX: {out}")
    print("Overwrite protection: ON | Use --force only for intentional replacement")
