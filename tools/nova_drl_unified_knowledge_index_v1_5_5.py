#!/usr/bin/env python3
"""
Nova DRL Base-PN Product Resolver + Complete Product View v1.5.5
...
- complete vertical parts ranking across the resolved base product,
- repair-history presentation shows only the top 10 repeated history patterns by distinct repair events,
...
def aggregate_product_parts(conn, families: Sequence[str]) -> List[Dict[str, Any]]:
    """Aggregate indexed replacement PNs across all suffix variants.

    Distinct repair-event IDs are unioned before counting so a single repair is
    never counted twice merely because its exact equipment-family label varied.
    """
    if not families:
        return []
    marks, params = _sql_in(families)
    rows = conn.execute(
        f"SELECT equipment_family,manufacturer_pn,display_name,event_ids_json,repair_event_count "
        f"FROM product_parts WHERE equipment_family IN ({marks})",
        params,
    ).fetchall()
    groups: Dict[str, Dict[str, Any]] = {}
    for r in rows:
        pn = compact(r["manufacturer_pn"])
        if not pn:
            # Main technician parts list is intentionally Part Number + count.
            continue
        key = alnum(pn)
        if not key:
            continue
        g = groups.setdefault(key, {"labels": collections.Counter(), "events": set()})
        g["labels"][pn] += max(1, int(r["repair_event_count"] or 0))
        try:
            evs = json.loads(r["event_ids_json"] or "[]")
        except Exception:
            evs = []
        g["events"].update(str(x) for x in evs if x)
    out: List[Dict[str, Any]] = []
    for key, g in groups.items():
        pn = g["labels"].most_common(1)[0][0]
        repairs = len(g["events"])
        out.append({
            "item_type": "product_part",
            "item_key": f"resolved_product_part:{key}",
            "primary_value": pn,
            "title": pn,
            "payload": {"pn": pn, "repairs": repairs, "event_ids": sorted(g["events"])},
        })
...
_HISTORY_ADMIN_RE = re.compile(
    r"(?:vendor\s*(?:&|and)\s*date\s*stickers?|final\s*o\.?\s*k\.?|"
    r"shipping\s*(?:charge|note)?|warranty\s+per\b|fa\s+rpt\s+required|"
    r"robot\s+fas\s+put\s+inside\s+packaging)", re.I,
)


def _history_norm(text: str) -> str:
    s = compact(text).casefold()
    ...
    stem = {
        ...
        "lubed": "lube", "lubricated": "lube", "lubricating": "lube",
        "adjusted": "adjust", "adjusting": "adjust", "tested": "test", "testing": "test",
        "belts": "belt", "bearings": "bearing", "capacitors": "capacitor", "motors": "motor",
        "seals": "seal", "fuses": "fuse", "boards": "board", "connectors": "connector",
    }
    for t in s.split():
...
def _history_snippets(row: Dict[str, Any]) -> List[str]:
    raw = compact(row.get("repair_history"))
    if not raw:
        return []
    test_norm = _history_norm(row.get("test_outcome") or "")
    chunks = re.split(r"\s*\|\s*|[\r\n]+", raw)
    out = []
    for c in chunks:
        c = compact(re.sub(r"^(?:repair\s+history|other\s+repair-history\s+notes?)\s*:\s*", "", c, flags=re.I))
        if len(c) < 8 or _HISTORY_ADMIN_RE.search(c):
            continue
        n = _history_norm(c)
        if len(n) < 6:
            continue
        if test_norm and (n == test_norm or (len(n) > 12 and n in test_norm)):
            continue
        if re.fullmatch(r"(?:pass|passed)?\s*all\s*test[s]?", n, re.I):
            continue
        out.append(c)
    return out
...
def aggregate_repair_history(events: Sequence[Dict[str, Any]], limit: int = PRODUCT_HISTORY_LIMIT) -> List[Dict[str, Any]]:
    """Return top recurring repair-history patterns using DRL's 80/20 rule."""
    candidates: List[Tuple[str, str, str]] = []
    for ev in events:
        eid = compact(ev.get("repair_event_id"))
        for text in _history_snippets(ev):
    ...
    for eid, text, norm in candidates:
        ...
        for c in clusters:
            ctoks = c["tokens"]
            # Technical tokens containing digits (part numbers, component IDs,
            # axis values) are anchors. Different anchors must not be merged just
            # because the surrounding repair wording is similar.
            ...
            if anchors and canchors and anchors.isdisjoint(canchors):
                continue
            if tokens and ctoks:
                inter = len(tokens & ctoks)
                union = len(tokens | ctoks)
                jacc = inter / union if union else 0.0
            else:
                jacc = 0.0
            if jacc < 0.45 and (tokens and ctoks and next(iter(tokens)) != next(iter(ctoks))):
                continue
            seq = difflib.SequenceMatcher(None, norm, c["norm"]).ratio()
            score = max(jacc, seq)
            if (jacc >= 0.68 or seq >= 0.84) and score > best_score:
        ...
        if best is None:
            ...
            best["events"].add(eid)
            best["labels"][text] += 1
            # Keep a representative close to the shortest common wording.
    ...
    for i, c in enumerate(clusters):
        ...
        rows.append({
            ...
            "item_key": f"history_pattern:{i}",
            "primary_value": label,
            "title": label,
            "payload": {"history": label, "repairs": count, "event_ids": sorted(c["events"])},
        })
...
def product_view_groups(conn, query: str, generic_groups: List[Tuple[str, List[Dict[str, Any]]]]) -> Optional[List[Tuple[str, List[Dict[str, Any]]]]]:
    ...
    # Preserve useful tracking/procurement/source-file hits from the fast generic
    # search, but replace fragmented product/parts/history sections with the
    # resolved full-product view.
...
def render_console(conn, query: str, args, *, show_pdf_hint: bool = True) -> int:
    ...
    for heading, subset in groups:
        ...
        if heading == "REPAIR HISTORY" and subset and display_type(subset[0]) == "history_pattern":
            print(f"{'TOP REPEATED REPAIR HISTORY':<68} {'TIMES SEEN':>10}")
            print(f"{'-' * 68} {'-' * 10}")
...
def pdf_report_blocks(query: str, groups: List[Tuple[str, List[Dict[str, Any]]]], elapsed_ms: float) -> List[Tuple[str, str]]:
    ...
    for heading, subset in groups:
        ...
        if heading == "PARTS REPLACED":
            ...
            blocks.append(("body", f"{'TOP REPEATED REPAIR HISTORY':<70} TIMES SEEN"))
...
def command_status(args) -> int:
    ...
    print(f"Presentation layer:    v{VERSION} | base-PN product resolver + vertical parts + top-10 repeated histories")
...
def command_plan(args) -> int:
    ...
    print("v1.5.5: base-PN product resolver + complete parts view + top-10 repeated repair histories")
    print("Search/database schema:          v1.5.3 full-corpus DB reused; v1.5.5 resolves base PN at query time")
