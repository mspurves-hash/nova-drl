#!/usr/bin/env python3
"""
Nova DRL Unified Knowledge Search + Print v1.4.13

Engineer-facing presentation layer over the v1.4.8 unified knowledge index.
...
The v1.4.8 builder remains the storage/index engine and is shipped alongside this
script. v1.4.13 reuses that DB and does not change the frozen repair corpus.
"""
...
def load_base():
    if not BASE_SCRIPT.exists():
        raise FileNotFoundError(f"Required v1.4.8 engine not found: {BASE_SCRIPT}")
    spec = importlib.util.spec_from_file_location("nova_drl_v148_engine", BASE_SCRIPT)
...
def render_console(conn, query: str, args, *, show_pdf_hint: bool = True) -> int:
    ...
    print("=" * 86)
    print("Coverage: file/path index = full DRL share | repair/parts/tracking knowledge = frozen 10% corpus")
    if not results:
...
def pdf_report_blocks(query: str, groups: List[Tuple[str, List[Dict[str, Any]]]], elapsed_ms: float) -> List[Tuple[str, str]]:
    """Build the engineer print view.

    v1.4.13 intentionally removes screen-oriented/redundant material from the
    printable report while leaving the unified search results unchanged:
    - Equipment/Product is promoted to the top and reduced to identity only.
    - Search timing/coverage is omitted from print.
    - Tracking prints RMA rows without repeated source paths; customer PO rows
      stay searchable but are omitted from the standard project printout.
    - The redundant Indexed Parts summary is omitted; event-bound Part
      Occurrences remains the printable replacement-parts list.
    """
    by_heading = {heading: subset for heading, subset in groups}
    blocks: List[Tuple[str, str]] = []

    # Product identity is the first business fact an engineer should see.
    products = by_heading.get("EQUIPMENT / PRODUCT", [])
    if products:
        blocks.append(("heading", "EQUIPMENT / PRODUCT"))
        for n, r in enumerate(products, 1):
            lines = item_lines(r)
            if lines:
                blocks.append(("body", f"{n}. {lines[0]}"))
    ...
    for heading in preferred_order:
        subset = by_heading.get(heading, [])
        if not subset:
            continue

        # Standard printable Tracking/Project is RMA-focused. Customer POs
        # remain indexed/searchable but are intentionally omitted here.
        if heading == "TRACKING / PROJECT":
            subset = [r for r in subset if display_type(r) == "rma"]
            if not subset:
                continue
        ...
        for n, r in enumerate(subset, 1):
            ...
            if not lines:
                continue
            if heading == "TRACKING / PROJECT":
                # RMA line already includes RMA, DRL log, and equipment.
                lines = lines[:1]
...
def command_status(args) -> int:
    # The storage engine is still v1.4.8 schema; v1.4.13 is the presentation/print layer.
...
def command_plan(args) -> int:
    ...
    print("v1.4.13 additions: cleaner engineer PDF layout; PowerShell 5.1 installer fixes; Windows auto-copy/open report workflow retained")
    print("Search/database schema:          unchanged from v1.4.8/v1.4.9 storage layer")
    print("AI/LLM calls for search/PDF/print: 0")
    return rc


def command_build(args) -> int:
    # Rebuild with the proven v1.4.8 engine. Setting VERSION changes its metadata/report label only.
...
def command_search(args, query: str) -> int:
    ...
    if not db.exists():
        print(f"ERROR: unified knowledge DB not built: {db}", file=sys.stderr)
        print("Run: python3 tools/nova_drl_unified_knowledge_index_v1_4_13.py --build")
        return 2
    conn = base.connect_ro(db)
    try:
        if args.json:
...
def make_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--tracking-root", default=base.DEFAULT_TRACKING_ROOT)
    p.add_argument("--benchmark-root", default=base.DEFAULT_BENCHMARK_ROOT)
