...
script. v1.4.10 reuses that DB and does not change the frozen repair corpus.
"""
...
def pdf_report_blocks(query: str, groups: List[Tuple[str, List[Dict[str, Any]]]], elapsed_ms: float) -> List[Tuple[str, str]]:
    ...
    blocks.append(("meta", "Coverage: full DRL file/path index; repair/parts/RMA/order knowledge from the current frozen 10% corpus"))
    for heading, subset in groups:
...
def create_pdf_for_query(conn, query: str, args) -> Path:
    ...
    if served:
        ...
        if urls:
            print("Open PDF:")
            for url in urls:
                print("  " + terminal_link(url))
        else:
            print(f"Report server is running on port {args.report_port}; use the server hostname/IP with {path.name}.")
    else:
...
def print_pdf_file(path: Path, args) -> bool:
    ...
    if lp:
        ...
        print("Use the clickable PDF link above to print from the engineer workstation.")
        return False
    try:
...
def command_status(args) -> int:
    # The storage engine is still v1.4.8 schema; v1.4.10 is the presentation/print layer.
    ...
    print(f"Presentation layer:    v{VERSION} | clean grouped output + strict identifier linking")
    ...
    print(f"Terminal actions:      blue :pdf / :print | clickable PDF URL when supported")
...
def command_plan(args) -> int:
    ...
    print("v1.4.10 presentation additions: blue action hints; clickable PDF URL; direct :print command")
...
def command_search(args, query: str) -> int:
    ...
    if not db.exists():
        print(f"ERROR: unified knowledge DB not built: {db}", file=sys.stderr)
        print("Run: python3 tools/nova_drl_unified_knowledge_index_v1_4_10.py --build")
        return 2
    conn = base.connect_ro(db)
    try:
        if args.json:
            ...
            return 0
        return render_console(conn, query, args)
    finally:
        conn.close()
    ...
    try:
        while True:
            ...
            if q == ":help":
                ...
                print(f"{blue(':pdf')}              Create a printable PDF for the last search and show a clickable browser link")
                print(f"{blue(':pdf <search>')}     Create a printable PDF directly for another search")
