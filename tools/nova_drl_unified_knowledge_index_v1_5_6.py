#!/usr/bin/env python3
"""
Nova DRL Component Core Resolver + Clean Failure/Repair View v1.5.6
...
- component-core resolver collapses recurring PN prefixes/suffixes/OCR variants inside a product using volume,
...
- both Reported Failure and Repair History show only the top 10 repeated patterns,
...
def _pn_clean(value: Any) -> str:
    return compact(value).upper().strip(" ,;:()[]{}")
...
def _numeric_sequences(value: Any) -> List[str]:
    return re.findall(r"\d{4,}", alnum(value))
...
def aggregate_product_parts(conn, families: Sequence[str]) -> List[Dict[str, Any]]:
    """80/20 component-core ranking across a resolved base product.

    The full replacement-mention table is used rather than pre-fragmented
    product_parts rows.  Core identities are intentionally conservative:
    - capacitance spellings (33uF / 33 µF) collapse,
    - a recurring 4+ digit core such as 7800 collapses HCPL7800/7800A/vendor
      order-style prefixes and suffixes when that core has volume support,
    - a rare one-digit OCR mutation may join a dominant numeric core,
    - otherwise the exact PN remains its own core.

    Counts are distinct repair events, never sums of already-aggregated rows.
    """
    if not families:
        return []
    marks, params = _sql_in(families)
    ...
    # Discover high-volume numeric cores from the product itself.  The exact
    # core PN is strong evidence; multiple differently-written PNs are also
    # strong evidence that prefixes/suffixes are incidental to DRL repair use.
    token_events: Dict[str, set] = collections.defaultdict(set)
    token_variants: Dict[str, set] = collections.defaultdict(set)
    exact_events: Dict[str, set] = collections.defaultdict(set)
    for r in rows:
        eid = compact(r.get("repair_event_id"))
        pn = _pn_clean(r.get("manufacturer_pn"))
        if not eid or _pn_placeholder(pn):
            continue
        pan = alnum(pn)
        exact_events[pan].add(eid)
        for token in _numeric_sequences(pn):
            token_events[token].add(eid)
            token_variants[token].add(pan)

    numeric_cores = []
    for token, evs in token_events.items():
        exact_support = len(exact_events.get(token, set()))
        if len(evs) >= 2 and (len(token_variants[token]) >= 2 or exact_support >= 2):
            numeric_cores.append((token, len(evs)))
    numeric_cores.sort(key=lambda x: (-x[1], -len(x[0]), x[0]))
    ...
    for r in rows:
        ...
        # Passive values are legitimate technician-facing identities even when
        # the extraction stored the value in text rather than manufacturer_pn.
        cap = _capacitance_core(raw_pn) or _capacitance_core(text)
        core = None
        core_kind = None
        if cap:
            core, core_kind = cap, "capacitance"
        elif not _pn_placeholder(raw_pn):
            pan = alnum(raw_pn)
            for token, support in numeric_cores:
                if token in pan:
                    core, core_kind = token, "numeric_core"
                    break
            if core is None:
                # Allow rare OCR mutations to join only a strongly supported
                # four-digit core.  This is intentionally 80/20 and product-local.
                rare_nums = _numeric_sequences(raw_pn)
                for token, support in numeric_cores:
                    if support < 5 or len(token) != 4:
                        continue
                    if any(len(n) == 4 and _one_digit_apart(n, token) for n in rare_nums):
                        core, core_kind = token, "numeric_core_fuzzy"
                        break
            if core is None:
                core, core_kind = raw_pn, "exact_pn"
        else:
            # No usable PN and no explicit capacitor value: preserve the raw
            # evidence in the corpus but do not invent a part number for the
            # simple technician Parts list.
            continue

        key = f"core:{alnum(core) or core.casefold()}"
        g = groups.setdefault(key, {
            "core": core, "kind": core_kind, "events": set(), "variants": collections.Counter(),
        })
        g["events"].add(eid)
    ...
    # Augment replacement rows with existing technician work-performed text.
    # The v1.5.2 extraction is intentionally 80/20; sometimes a tech wrote
    # "replaced 7800/caps" in Repair History even when the parts extractor did
    # not create a separate replacement row.  We can recover that occurrence
    # without rereading a Traveler because the structured event text is already
    # in the full knowledge DB.
    ...
    def add_history_core(core: str, kind: str, eid: str, evidence: str) -> None:
        key = f"core:{alnum(core) or core.casefold()}"
        g = groups.setdefault(key, {
            "core": core, "kind": kind, "events": set(), "variants": collections.Counter(),
        })
        g["events"].add(eid)
        if evidence:
            g["variants"][evidence] += 0
    ...
    for ev in ev_rows:
        ...
        for snippet in _technician_repair_snippets(ev):
            for cap_core in _capacitance_cores(snippet):
                add_history_core(cap_core, "capacitance", eid, snippet)
            san = alnum(snippet)
            for token, support in numeric_cores:
                if token in san:
                    add_history_core(token, "numeric_core", eid, snippet)
    ...
    for key, g in groups.items():
        ...
        out.append({
            ...
            "item_key": f"core_product_part:{key}",
            "primary_value": label,
            "title": label,
            "payload": {
                "pn": label,
                ...
                "core_kind": g["kind"],
                "observed_variants": [x for x, _ in g["variants"].most_common()],
            },
        })
...
def _strip_field_prefix(text: str) -> str:
    return compact(re.sub(
        r"^(?:repair\s+history|other\s+repair-history\s+notes?|reported\s+problem|customer\s+complaint)\s*:\s*",
        "", text, flags=re.I,
    ))
...
def product_view_groups(conn, query: str, generic_groups: List[Tuple[str, List[Dict[str, Any]]]]) -> Optional[List[Tuple[str, List[Dict[str, Any]]]]]:
    ...
    parts = aggregate_product_parts(conn, resolved["families"])
    failures = aggregate_reported_failures(events)
    histories = aggregate_repair_history(events)
    product = {
        "item_type": "product",
    ...
    generic = {h: rows for h, rows in generic_groups}
    out: List[Tuple[str, List[Dict[str, Any]]]] = []
    for heading in ("TRACKING / PROJECT", "PROCUREMENT / REORDER"):
        if generic.get(heading):
            out.append((heading, generic[heading]))
    ...
    if histories:
        out.append(("REPAIR HISTORY", histories))
    if generic.get("SOURCE FILES"):
        out.append(("SOURCE FILES", generic["SOURCE FILES"]))
    return out
...
def pdf_report_blocks(query: str, groups: List[Tuple[str, List[Dict[str, Any]]]], elapsed_ms: float) -> List[Tuple[str, str]]:
    ...
    for heading, subset in groups:
        ...
        if heading == "PARTS REPLACED":
            blocks.append(("body", f"{'PART NUMBER':<54} TIMES REPLACED"))
            blocks.append(("body", f"{'-' * 54} --------------"))
            for r in subset:
                p = r.get("payload") or {}
                pn = compact(p.get("pn") or r.get("primary_value") or r.get("title")) or "-"
                repairs = int(p.get("repairs") or 0)
                blocks.append(("body", f"{pn[:54]:<54} {repairs}"))
        elif heading == "REPORTED FAILURE" and subset and display_type(subset[0]) == "failure_pattern":
            blocks.append(("body", f"{'TOP REPORTED FAILURES':<70} TIMES SEEN"))
            ...
            for r in subset[:PRODUCT_FAILURE_LIMIT]:
                ...
                repairs = int(p.get("repairs") or 0)
                blocks.append(("body", f"{failure[:70]:<70} {repairs}"))
...
def command_status(args) -> int:
    ...
    print(f"Presentation layer:    v{VERSION} | component-core parts + reported failures + technician repair history")
...
def command_plan(args) -> int:
    ...
    print("v1.5.6: component-core resolver + clean failure/technician-repair product view")
    print("Search/database schema:          v1.5.3 full-corpus DB reused; v1.5.6 resolves product + component cores at query time")
