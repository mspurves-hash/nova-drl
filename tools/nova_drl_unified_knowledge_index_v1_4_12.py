#!/usr/bin/env python3
"""
Nova DRL Unified Knowledge Search + Print v1.4.12
...
script. v1.4.12 reuses that DB and does not change the frozen repair corpus.
"""
...
def command_status(args) -> int:
    # The storage engine is still v1.4.8 schema; v1.4.12 is the presentation/print layer.
...
def command_plan(args) -> int:
    ...
    print("v1.4.12 additions: Windows Engineer Client; auto-copy/open PDF; Base64-safe remote queries; existing browser/print fallbacks retained")
...
def command_search(args, query: str) -> int:
    ...
    if not db.exists():
        print(f"ERROR: unified knowledge DB not built: {db}", file=sys.stderr)
        print("Run: python3 tools/nova_drl_unified_knowledge_index_v1_4_12.py --build")
        return 2
    conn = base.connect_ro(db)
    try:
        if args.json:
