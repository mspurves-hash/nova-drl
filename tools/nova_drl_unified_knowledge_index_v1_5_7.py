#!/usr/bin/env python3
"""
Nova DRL Minimal Product Report — Corpus-Only 80/20 View v1.5.7
...
def aggregate_product_parts(conn, families: Sequence[str], base_part_number: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    # HARD 80/20 RULE: no expert/user-provided repair-kit projection is permitted here.
    # Counts below are the union of corpus-supported repair-event evidence only.
    ...
    for key, g in groups.items():
        ...
        if repairs < PRODUCT_PART_MIN_REPAIRS:
            # 80/20 product view: one-off strings remain preserved/searchable
            # in the underlying corpus but are normally OCR/noise or rare work
            # and do not belong in the recurring technician parts list.
            continue
        out.append({
...
def render_console(conn, query: str, args, *, show_pdf_hint: bool = True) -> int:
    ...
    for heading, subset in groups:
        ...
        print("-" * len(heading))
        if heading == "PARTS REPLACED":
            print(f"{'PART NUMBER':<52}    {'TIMES REPLACED':>16}")
            print(f"{'-' * 52}    {'-' * 16}")
...
def pdf_report_blocks(query: str, groups: List[Tuple[str, List[Dict[str, Any]]]], elapsed_ms: float) -> List[Tuple[str, str]]:
    ...
    for heading, subset in groups:
        ...
        if heading == "PARTS REPLACED":
            blocks.append(("parts_header", "PART NUMBER\tTIMES REPLACED"))
            for r in subset:
                p = r.get("payload") or {}
...
def command_plan(args) -> int:
    ...
    print("v1.5.7: corpus-only 80/20 component-core resolver + minimal product view")
    print("Search/database schema:          v1.5.3 full-corpus DB reused; v1.5.7 resolves product + component cores at query time")
