#!/usr/bin/env python3
"""
Nova DRL Global Evidence Rollup — Corpus-Only Product View v1.5.11
...
- GLOBAL volume-based product resolver aggregates punctuation/suffix/spelling/customer variants into one product view by total repair-event coverage,
...
- recurring complete alphanumeric PNs are preferred over bare numeric fragments when the corpus supports them, while strongly dominant stable numeric cores remain valid when volume clearly supports the shorter reference,
...
- normal product view is concise: product identity + recurring Parts Replaced + Reported Failure + recurring Repair Actions,
- recurring Parts Replaced includes useful component/assembly names when no manufacturer PN was recorded,
- raw individual Repair History remains omitted; only recurring action/component signals are surfaced,
...
GENERIC_COMPONENT_NAME_RECOVERY = True
RECURRING_REPAIR_ACTIONS_80_20 = True
...
GENERIC_ROOT_CAUSE_FIXES_MUST_BE_GLOBAL = True
ALLOW_PRODUCT_SPECIFIC_RESOLVER_PATCHES = False
...
def enforce_drl_project_invariants() -> None:
    ...
    if GENERIC_COMPONENT_NAME_RECOVERY is not True:
        raise RuntimeError("DRL Nova policy violation: recurring non-PN component recovery must remain global")
    if RECURRING_REPAIR_ACTIONS_80_20 is not True:
        raise RuntimeError("DRL Nova policy violation: recurring repair-action rollup must remain enabled")
    if GENERIC_ROOT_CAUSE_FIXES_MUST_BE_GLOBAL is not True:
        raise RuntimeError("DRL Nova policy violation: proven generic fixes must be global")
    if ALLOW_PRODUCT_SPECIFIC_RESOLVER_PATCHES is not False:
        raise RuntimeError("DRL Nova policy violation: product-specific resolver patches are forbidden unless explicitly promoted as a true exception")
...
def _model_token_norm(value: Any) -> str:
    return alnum(compact(value).upper())


def _raw_suffix_boundary(base: str, variant: str) -> bool:
    """Return True when VARIANT visibly continues BASE as a suffix form.

    This is deliberately manufacturer-agnostic.  A suffix may be separated by
    punctuation (e.g. BASE-01) or may begin with an alphabetic variant marker
    immediately after the base (e.g. BASEB-CE).  Pure numeric extensions with no
    delimiter are *not* assumed equivalent because MODEL-100 and MODEL-1000 may
    be distinct base products.
    """
    b = re.sub(r"\s+", "", compact(base).upper())
    v = re.sub(r"\s+", "", compact(variant).upper())
    if not b or not v or v == b:
        return v == b
    if v.startswith(b):
        rem = v[len(b):]
        if not rem:
...
def _model_variant_of(base: str, variant: str) -> bool:
    """Generic DRL repair-equivalence test for model suffix variants.

    Product grouping uses a shorter *observed* base model only when a longer
    observed model is a suffix/punctuation continuation of that base.  This is
    global logic: no product name, OEM, or known model is encoded here.
    """
    bn = _model_token_norm(base)
    vn = _model_token_norm(variant)
    if not bn or not vn:
        return False
    if bn == vn:
        return True
    if not vn.startswith(bn):
        return False
    rem = vn[len(bn):]
    if not rem:
        return True
    # Alphabetic-leading normalized suffixes are conventional variant markers.
    if rem[0].isalpha():
        return True
    # Numeric-leading suffixes require a visible delimiter in the observed form.
    return _raw_suffix_boundary(base, variant)


def resolve_base_product(conn, query: str) -> Optional[Dict[str, Any]]:
    """Resolve a query to the globally recurring DRL base repair product.

    Every *observed* model token relevant to the query is evaluated as a possible
    base.  The winning base is the candidate that explains the greatest number of
    repair events across its generic suffix/punctuation variants.  Equal-coverage
    candidates prefer the shorter observed base.  Exact spellings remain metadata.
    """
    q = compact(query)
    qn = _model_token_norm(q)
    ...
    # De-duplicate candidate model spellings while retaining their total direct
    # event volume.  Product-family rows can repeat the same model token because
    # OEM/customer spelling differs.
    model_rows: Dict[str, Dict[str, Any]] = {}
    for c in candidates:
        key = c["model_norm"]
        g = model_rows.setdefault(key, {"model": c["model"], "events": 0, "rows": []})
        g["events"] += c["events"]
        g["rows"].append(c)
        # Prefer the shorter punctuation-preserving spelling for equivalent norm.
        if len(c["model"]) < len(g["model"]):
            g["model"] = c["model"]

    scored = []
    for g in model_rows.values():
        root = g["model"]
        members = [c for c in catalog if _model_variant_of(root, c["model"])]
        coverage = sum(c["events"] for c in members)
        exact_query = 1 if _model_token_norm(root) == qn else 0
        scored.append((coverage, -len(_model_token_norm(root)), exact_query, g["events"], root, members))

    if not scored:
        return None
    coverage, _, _, _, root, variants = max(scored, key=lambda x: x[:4])
    if not variants:
        return None

    root_norm = _model_token_norm(root)
    families = sorted({c["family"] for c in variants}, key=str.casefold)
    exact = [c for c in variants if _model_token_norm(c["model"]) == root_norm]
    ...
    return {
        ...
        "resolved_repair_events": int(coverage),
    }
...
def _text_pn_candidates(text: Any, raw_norms: set[str]) -> List[str]:
    ...
    # Mixed alphanumeric candidates, including mixed alpha/numeric, punctuation, and distributor-prefix forms.
    patterns = [
    ...
    # Numeric-only core tokens are allowed from text only when
...
# Generic non-PN component recovery. Many mechanical/robot Travelers record a
# useful assembly name (for example an axis motor, encoder, belt, lead screw,
# sensor, board, etc.) without a manufacturer PN. These names are legitimate
# recurring parts evidence and must not disappear merely because a PN is absent.
# The vocabulary is deliberately equipment-agnostic and contains no product
# names or known product-specific mappings.
_PART_REPLACEMENT_ACTION_RE = re.compile(
    r"(?:replac(?:e|ed|ing)|chang(?:e|ed|ing)|install(?:ed|ing)?|swap(?:ped|ping)?|new\b)", re.I,
)
_NON_REPLACEMENT_SEGMENT_RE = re.compile(
    r"(?:reus(?:e|ed|ing)|clean(?:ed|ing)?|adjust(?:ed|ing)?|align(?:ed|ing)?|"
    r"test(?:ed|ing)?|check(?:ed|ing)?|set\b|lub(?:e|ed)|greas(?:e|ed|ing))", re.I,
)
_COMPONENT_TOKEN_ALIASES = {
    "MOTORS":"MOTOR", "ENCODERS":"ENCODER", "BELTS":"BELT", "SCREWS":"SCREW",
    "BEARINGS":"BEARING", "SENSORS":"SENSOR", "SOLENOIDS":"SOLENOID",
    "BOARDS":"BOARD", "RELAYS":"RELAY", "CONNECTORS":"CONNECTOR",
    "CAPS":"CAPACITOR", "CAPACITORS":"CAPACITOR", "FUSES":"FUSE",
    "TRANSISTORS":"TRANSISTOR", "DIODES":"DIODE", "FANS":"FAN",
    "PULLEYS":"PULLEY", "VALVES":"VALVE", "SEALS":"SEAL",
    "BUSHINGS":"BUSHING", "CONTROLLERS":"CONTROLLER", "RESISTORS":"RESISTOR",
    "TRANSFORMERS":"TRANSFORMER", "HARNESSES":"HARNESS", "CABLES":"CABLE",
    "SWITCHES":"SWITCH", "CONTACTORS":"CONTACTOR", "GEARS":"GEAR",
    "ROLLERS":"ROLLER", "SPRINGS":"SPRING", "BRAKES":"BRAKE",
    "COUPLINGS":"COUPLING", "PINS":"PIN", "CCDS":"CCD", "LEDS":"LED",
}
_COMPONENT_NOUNS = {
    "MOTOR","ENCODER","BELT","SCREW","BEARING","CCD","LED","SENSOR","SOLENOID",
    "BOARD","PCB","RELAY","CONNECTOR","CAPACITOR","FUSE","TRANSISTOR","MOSFET",
    "DIODE","FAN","CHUCK","PULLEY","VALVE","SEAL","BUSHING","CONTROLLER",
    "RESISTOR","TRANSFORMER","HARNESS","CABLE","SWITCH","CONTACTOR","GEAR",
    "ROLLER","SPRING","BRAKE","COUPLING","PIN","IC","CHIP","LAMP","LIGHT",
}
_COMPONENT_AXIS = {"X","Y","Z","R","T","THETA"}
_COMPONENT_QUALIFIERS = _COMPONENT_AXIS | {
    "LEAD","BALL","LADDER","PIN","CHUCK","VAC","VACUUM","MAIN","TOP","BOTTOM",
    "CCD","LED","ARM","HOME","LIMIT","DRIVE","POWER","CONTROL","SERVO",
}
...
def _component_reference_signals(text: Any) -> List[str]:
    """Return concise, generic component/assembly references visible in text.

    This does not infer a PN. It only promotes an explicitly named component
    from structured replacement/technician evidence. Axis/assembly qualifiers
    are preserved when they materially distinguish the component.
    """
    s = compact(text).upper().replace("µ", "U").replace("μ", "U")
    if not s:
        return []
    s = re.sub(r"\bLEAD\s*SCREWS?\b", "LEAD SCREW", s)
    s = re.sub(r"\bBALL\s*SCREWS?\b", "BALL SCREW", s)
    toks = re.findall(r"[A-Z0-9]+", s)
    toks = [_COMPONENT_TOKEN_ALIASES.get(t, t) for t in toks]
    out: List[str] = []
    for i, noun in enumerate(toks):
        if noun not in _COMPONENT_NOUNS:
            continue
        # A component word used as a qualifier for a following component should
        # not also become a separate generic row (e.g. LED BOARD, PIN MOTOR).
        if i + 1 < len(toks) and noun in _COMPONENT_QUALIFIERS and toks[i+1] in _COMPONENT_NOUNS:
            continue
        qualifier = None
        if noun == "SCREW" and i > 0 and toks[i-1] in {"LEAD","BALL"}:
            qualifier = toks[i-1]
            if i > 1 and toks[i-2] in _COMPONENT_AXIS:
                qualifier = toks[i-2] + " " + qualifier
        elif i > 0 and toks[i-1] == "AXIS" and i > 1 and toks[i-2] in _COMPONENT_AXIS:
            qualifier = toks[i-2]
        elif i > 0 and toks[i-1] in _COMPONENT_QUALIFIERS:
            qualifier = toks[i-1]
        label = f"{qualifier} {noun}" if qualifier else noun
        label = _component_norm(label)
        if label and label not in out:
            out.append(label)
    specific_nouns = {x.split()[-1] for x in out if len(x.split()) > 1}
    return [x for x in out if not (len(x.split()) == 1 and x in specific_nouns)]


def _split_action_segments(text: Any) -> List[str]:
    s = compact(text)
    if not s:
        return []
    s = re.sub(
        r"\s+(?:AND|THEN)\s+(?=(?:REPLAC|CHANG|INSTALL|SWAP|CLEAN|LUB|GREAS|ADJUST|ALIGN|REPAIR|REBUILD|FIX|TIGHTEN|SHIM|FLUSH))",
        " | ", s, flags=re.I,
    )
    return [compact(x) for x in re.split(r"\s*[|;/]\s*|[\r\n]+", s) if compact(x)]


def _replacement_component_signals(text: Any, *, assume_explicit_part: bool) -> List[str]:
    out: List[str] = []
    for seg in _split_action_segments(text) or [compact(text)]:
        has_replace = bool(_PART_REPLACEMENT_ACTION_RE.search(seg))
        if not assume_explicit_part and not has_replace:
            continue
        if assume_explicit_part and _NON_REPLACEMENT_SEGMENT_RE.search(seg) and not has_replace:
            continue
        for comp in _component_reference_signals(seg):
            if comp not in out:
                out.append(comp)
    return out


_REPAIR_ACTION_CLASS_RULES = [
    ("REPLACE", re.compile(r"(?:replac(?:e|ed|ing)|chang(?:e|ed|ing)|install(?:ed|ing)?|swap(?:ped|ping)?)", re.I)),
    ("REBUILD", re.compile(r"(?:rebuild|rebuilt|reconstruct(?:ed|ion)?)", re.I)),
    ("CLEAN", re.compile(r"(?:clean(?:ed|ing)?|flush(?:ed|ing)?)", re.I)),
    ("LUBRICATE", re.compile(r"(?:lub(?:e|ed|ing)?|lubricat(?:e|ed|ing)|greas(?:e|ed|ing)?)", re.I)),
    ("ADJUST", re.compile(r"(?:adjust(?:ed|ing)?|shim(?:med|ming)?|tighten(?:ed|ing)?|tension(?:ed|ing)?|level(?:ed|ing)?)", re.I)),
    ("ALIGN", re.compile(r"(?:align(?:ed|ing)?)", re.I)),
    ("REPAIR", re.compile(r"(?:repair(?:ed|ing)?|fix(?:ed|ing)?|rework|resolder|solder(?:ed|ing)?|reflow(?:ed|ing)?)", re.I)),
]


def _repair_action_class(text: str) -> Optional[str]:
    for label, rx in _REPAIR_ACTION_CLASS_RULES:
        if rx.search(text):
            return label
    return None
...
def _digit_groups(value: Any) -> Tuple[str, ...]:
    """Digit runs with leading-zero normalization for OCR comparison."""
    out = []
    for g in re.findall(r"\d+", _pn_compare_norm(value)):
        try:
...
def _meaningful_alpha_count(value: Any) -> int:
    return sum(ch.isalpha() for ch in _pn_compare_norm(value))


def _variant_related(a: str, b: str) -> bool:
    """Generic product-local equivalence test for observed PN spellings.

    The relation uses only shape, digit groups, alphabetic structure, edit
    similarity, and containment.  No product or part-number literal appears in
    the resolver.
    """
    ...
    a_num = an.isdigit(); b_num = bn.isdigit()
    short, long = (an, bn) if len(an) <= len(bn) else (bn, an)
    if len(short) >= 4 and short in long and len(short) / max(1, len(long)) >= 0.42:
        return True

    adg, bdg = _digit_groups(an), _digit_groups(bn)
    digit_chars = sum(len(x) for x in adg) if adg else 0
    seq = difflib.SequenceMatcher(None, an, bn).ratio()

    # A recurring numeric core may join a complete alphanumeric spelling when
    # the same substantial digit identity is present. Numeric-to-numeric OCR
    # mutations are intentionally *not* joined here.
    if adg and bdg and adg == bdg and digit_chars >= 4:
        if a_num != b_num:
            return True
        if not a_num and not b_num and seq >= 0.50:
            return True

    aa, ba = _alpha_signature(an), _alpha_signature(bn)
    if not a_num and not b_num:
        if aa and ba and aa == ba and seq >= 0.72:
            return True
        if seq >= 0.86:
            return True
        # One OCR digit substitution is allowed only when alphabetic structure
        # remains strongly similar.
        if adg and bdg and len(adg) == len(bdg) and len(aa) >= 2 and len(ba) >= 2:
            diffs = sum(x != y for x, y in zip(adg, bdg))
            if diffs == 1 and difflib.SequenceMatcher(None, aa, ba).ratio() >= 0.80 and seq >= 0.78:
                return True
    return False
...
def _display_reference_pn(cluster: Dict[str, Any]) -> Optional[str]:
    """Choose a corpus-only technician-facing REFERENCE PN.

    The winning label is always an observed variant.  Complete recurring
    alphanumeric forms are preferred over bare numeric fragments when their
    event support is at least as strong; a truly dominant numeric core remains
    valid when full forms are sparse.  Distributor-style numeric prefixes are
    penalized for display but remain preserved as evidence.
    """
    if cluster.get("kind") == "capacitance":
        return cluster.get("cap")
    if cluster.get("kind") == "component":
        return cluster.get("component")

    stats = []
    for norm, info in cluster["variants"].items():
        support = len(info["events"])
        display = _candidate_display(info["display"].most_common(1)[0][0])
        cmpn = _pn_compare_norm(display)
        if not cmpn or not _pn_candidate_usable(display):
            continue
        alpha_count = _meaningful_alpha_count(display)
        catalog_prefix = bool(re.match(r"^\d{3}[- ]?[A-Z]", display, flags=re.I))
        stats.append({
            "norm": norm, "cmp": cmpn, "display": display,
            "support": support, "events": set(info["events"]),
            "numeric": cmpn.isdigit(), "alpha_count": alpha_count,
            "length": len(cmpn), "catalog_prefix": catalog_prefix,
        })
    if not stats:
        return None

    numeric = [c for c in stats if c["numeric"]]
    full = [c for c in stats if not c["numeric"] and c["alpha_count"] >= 2]
    dominant = max(stats, key=lambda c: (c["support"], not c["numeric"], c["alpha_count"], c["length"]))

    if dominant["numeric"] and full:
        full_events = set().union(*(c["events"] for c in full)) if full else set()
        best_full = max(
            full,
            key=lambda c: (
                c["support"],
                not c["catalog_prefix"],
                c["alpha_count"],
                c["length"],
            ),
        )
        # Promote only when complete spellings collectively explain at least as
        # many repair events as the numeric core and one full spelling recurs.
        if len(full_events) >= dominant["support"] and best_full["support"] >= 2:
            dominant = best_full

    # If the dominant form is already alphanumeric, allow a longer recurring
    # complete observed spelling to win when it explains meaningful volume and
    # is not merely a distributor/catalog prefix.
    if not dominant["numeric"]:
        threshold = max(2, (dominant["support"] + 3) // 4)
        expansions = [
            c for c in full
            if c["support"] >= threshold
            and _variant_related(dominant["display"], c["display"])
            and c["length"] >= dominant["length"]
            and c["alpha_count"] >= dominant["alpha_count"]
            and not c["catalog_prefix"]
        ]
        if expansions:
            dominant = max(expansions, key=lambda c: (c["length"], c["alpha_count"], c["support"]))

    return dominant["display"]
...
def aggregate_product_parts(conn, families: Sequence[str], base_part_number: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    v1.5.11 keeps event-level counting and the corpus-only Reference-PN resolver,
    while globally recovering recurring component/assembly names that have no PN:
    - primary evidence comes from explicit structured `parts_replaced` rows;
    - explicit replacement/change/install/swap wording already stored in technician
      Repair History may recover a replacement fact missed by the parts classifier;
    - raw PN and replacement text can both contribute PN signals;
    - recurring non-PN component/assembly names are valid technician references;
    ...
    def add_signal(display: str, eid: str, *, kind: str = "pn") -> None:
        ...
        if kind == "capacitance":
            ...
            shown = _component_norm(display)
            if not shown:
                return
            key = f"COMP:{shown}"
            n = nodes.setdefault(key, {"kind": kind, "component": shown, "events": set(), "display": collections.Counter()})
        else:
            ...
            shown = _candidate_display(display)
        n["events"].add(eid)
        n["display"][shown] += 1
    ...
    for r in rows:
        ...
        caps = _capacitance_cores(" ".join(x for x in (raw_pn, text) if x))
        text_pns = _text_pn_candidates(text, raw_norms)
        for cap in caps:
            add_signal(cap, eid, kind="capacitance")
        usable_raw = bool(raw_pn and not _pn_placeholder(raw_pn) and _pn_candidate_usable(raw_pn))
        if usable_raw:
            add_signal(raw_pn, eid)
        for cand in text_pns:
            if not _capacitance_core(cand):
                add_signal(cand, eid)
        if not caps and not usable_raw and not text_pns:
            for comp in _replacement_component_signals(text, assume_explicit_part=True):
                add_signal(comp, eid, kind="component")

    # Recover explicit replacement facts that the v1.5.2 reasoner sometimes
    # placed in technician Repair History instead of parts_replaced. Only
    # replacement/change/install/swap wording is allowed into the Parts table;
    # cleaning/alignment/adjustment stays in recurring Repair Actions below.
    ev_rows = [dict(r) for r in conn.execute(
        f"SELECT repair_event_id,repair_history_text AS repair_history,test_outcome_text AS test_outcome "
        f"FROM repair_events WHERE equipment_family IN ({marks})", params,
    ).fetchall()]
    for ev in ev_rows:
        eid = compact(ev.get("repair_event_id"))
        if not eid:
            continue
        for snippet in _technician_repair_snippets(ev):
            for seg in _split_action_segments(snippet):
                if not _PART_REPLACEMENT_ACTION_RE.search(seg):
                    continue
                caps = _capacitance_cores(seg)
                pns = _text_pn_candidates(seg, raw_norms)
                for cap in caps:
                    add_signal(cap, eid, kind="capacitance")
                for cand in pns:
                    if not _capacitance_core(cand):
                        add_signal(cand, eid)
                if not caps and not pns:
                    for comp in _replacement_component_signals(seg, assume_explicit_part=False):
                        add_signal(comp, eid, kind="component")
    ...
    for key, node in nodes.items():
        ...
        c = clusters.setdefault(root, {
            "kind": node["kind"], "cap": node.get("cap"), "component": node.get("component"), "events": set(),
        ...
        if node["kind"] == "pn":
            ...
            c["component"] = node.get("component") or c.get("component")
    ...
    for root, c in clusters.items():
        ...
        out.append({
            ...
            "payload": {
                ...
                "source_policy": "explicit_structured_parts_or_explicit_replacement_history_reference_80_20",
                "reference_kind": c.get("kind"),
...
def aggregate_repair_actions(events: Sequence[Dict[str, Any]], limit: int = PRODUCT_HISTORY_LIMIT) -> List[Dict[str, Any]]:
    """Global 80/20 rollup of explicit technician work performed.

    Raw individual history remains hidden in the normal product view. Only an
    action + component/reference repeated in at least two distinct repair events
    is surfaced. This keeps mechanical/robot products useful without returning
    the noisy free-text Repair History section that was intentionally removed.
    """
    ...
    for ev in events:
        ...
        for snippet in _technician_repair_snippets(ev):
            for seg in _split_action_segments(snippet):
                action = _repair_action_class(seg)
                if not action:
                    continue
                refs = _component_reference_signals(seg)
                if not refs:
                    refs = _capacitance_cores(seg)
                if not refs:
                    refs = _text_pn_candidates(seg, set())
                for ref in refs:
                    comp_refs = _component_reference_signals(ref)
                    shown = _component_norm(ref) if comp_refs else _candidate_display(ref)
                    shown = shown or compact(ref)
                    if not shown:
                        continue
                    key = f"{action}:{_component_norm(shown) or alnum(shown)}"
                    g = groups.setdefault(key, {"action": action, "reference": shown, "events": set()})
                    g["events"].add(eid)
    rows: List[Dict[str, Any]] = []
    for key, g in groups.items():
        repairs = len(g["events"])
        if repairs < 2:
            continue
        label = f"{g['action']} {g['reference']}"
        rows.append({
            "item_type": "history_pattern",
            "item_key": f"repair_action:{alnum(key)}",
            "primary_value": label,
            "title": label,
            "payload": {
                "history": label, "repair_action": label, "repairs": repairs,
                "event_ids": sorted(g["events"]),
                "source_policy": "recurring_explicit_technician_action_component_80_20",
            },
        })
    rows.sort(key=lambda r: (-int(r["payload"]["repairs"]), compact(r["primary_value"]).casefold()))
    return rows[:limit]
...
def product_view_groups(conn, query: str, generic_groups: List[Tuple[str, List[Dict[str, Any]]]]) -> Optional[List[Tuple[str, List[Dict[str, Any]]]]]:
    ...
    repair_actions = aggregate_repair_actions(events)
    product = {
        "item_type": "product",
    ...
    if repair_actions:
        out.append(("RECURRING REPAIR ACTIONS", repair_actions))
    return out
...
def render_console(conn, query: str, args, *, show_pdf_hint: bool = True) -> int:
    ...
    for heading, subset in groups:
        ...
        print("-" * len(heading))
        if heading == "PARTS REPLACED":
            print(f"{'REFERENCE PN / COMPONENT':<52}    {'TIMES REPLACED':>16}")
            print(f"{'-' * 52}    {'-' * 16}")
        ...
        if heading == "REPORTED FAILURE" and subset and display_type(subset[0]) == "failure_pattern":
            ...
            continue
        if heading in {"REPAIR HISTORY", "RECURRING REPAIR ACTIONS"} and subset and display_type(subset[0]) == "history_pattern":
            print(f"{'RECURRING REPAIR ACTION':<68} {'TIMES SEEN':>10}")
            print(f"{'-' * 68} {'-' * 10}")
...
def pdf_report_blocks(query: str, groups: List[Tuple[str, List[Dict[str, Any]]]], elapsed_ms: float) -> List[Tuple[str, str]]:
    ...
    for heading, subset in groups:
        ...
        if heading == "PARTS REPLACED":
            blocks.append(("parts_header", "REFERENCE PN / COMPONENT\tTIMES REPLACED"))
            for r in subset:
                p = r.get("payload") or {}
        ...
        elif heading in {"REPAIR HISTORY", "RECURRING REPAIR ACTIONS"} and subset and display_type(subset[0]) == "history_pattern":
            blocks.append(("body", f"{'RECURRING REPAIR ACTION':<70} TIMES SEEN"))
...
def command_status(args) -> int:
    ...
    print(f"Presentation layer:    v{VERSION} | recurring PN/components + reported failures + repair actions")
...
def command_plan(args) -> int:
    ...
    print("v1.5.11: global component-name recovery + recurring repair-action rollup + corpus-only reference resolver")
    print("Search/database schema:          v1.5.3 full-corpus DB reused; v1.5.11 resolves product + PN/component references and recurring repair actions from structured corpus evidence at query time")
