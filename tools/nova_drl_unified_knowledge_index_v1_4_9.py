...
- no AI/LLM call for ordinary search or PDF generation.

The v1.4.8 builder remains the storage/index engine and is shipped alongside this
script. v1.4.9 reuses that DB and does not change the frozen repair corpus.
"""
    ...
    for heading, subset in groups:
        ...
        for i, result in enumerate(subset, 1):
            ...
            for line in lines[1:]:
                print(f"   {line}")
    if show_pdf_hint:
        print("\nPrint/Save: type :pdf to create a printable PDF of this search.")
    return 0
...
def command_status(args) -> int:
    # The storage engine is still v1.4.8 schema; v1.4.9 is the presentation/print layer.
...
def command_plan(args) -> int:
    ...
    print("v1.4.9 presentation additions: customer PO separation; Picasa suppression; strict identifier linking; printable PDF")
    print("AI/LLM calls for search/PDF:   0")
    return rc
...
def command_search(args, query: str) -> int:
    ...
    if not db.exists():
        print(f"ERROR: unified knowledge DB not built: {db}", file=sys.stderr)
        print("Run: python3 tools/nova_drl_unified_knowledge_index_v1_4_9.py --build")
        return 2
    conn = base.connect_ro(db)
    try:
        if args.json:
...
def command_prompt(args) -> int:
    ...
    print("Simple lookup is local/indexed; no AI call. Commands: :help  :status  :pdf  :quit")
    try:
        while True:
            try:
            ...
            if q == ":help":
                ...
                print(":pdf              Create a printable PDF for the last search and print a browser link")
                print(":pdf <search>     Create a printable PDF directly for another search")
