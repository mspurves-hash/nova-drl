#!/usr/bin/env python3
"""
Nova DRL Structured-Event Parts Aggregation — Corpus-Only 80/20 View v1.5.8
...
def aggregate_product_parts(conn, families: Sequence[str], base_part_number: Optional[str] = None) -> List[Dict[str, Any]]:
    """80/20 component-core ranking from explicit structured parts_replaced evidence.

    v1.5.8 deliberately treats the flattened replacement_mentions table as the
    structured `facts.parts_replaced` source, but DOES NOT require the model to
    have populated manufacturer_pn.  The replacement text itself is valid
    explicit replacement evidence and may contain the recurring core PN.

    Rules:
    - counts are DISTINCT repair events, never summed already-aggregated rows;
    - manufacturer_pn and replacement text both participate in core discovery;
    - capacitance spellings (33uF / 33 µF) collapse;
    - recurring 4+ digit cores (e.g. 7800) collapse product-local prefix/suffix
      and OCR variants when the corpus itself provides recurrence;
    - one-off strings remain searchable in the underlying corpus but are hidden
      from the normal 80/20 product view;
    - NO technician repair-history text is used to inflate part counts here;
      the source is explicit `parts_replaced` evidence only;
    - NO expert/user repair-kit projection is permitted.
    """
    if not families:
        return []
    marks, params = _sql_in(families)
    ...
    # Discover high-volume numeric cores across BOTH the PN field and explicit
    # replacement text.  This fixes the important case where a structured
    # parts_replaced row says "Replaced HCPL-7800..." but part_number is blank.
    ...
    for r in rows:
        ...
        if pn and not _pn_placeholder(pn):
            pan = alnum(pn)
            exact_events[pan].add(eid)
        combined = " ".join(x for x in (pn if not _pn_placeholder(pn) else "", text) if x)
        for token in _numeric_sequences(combined):
            token_events[token].add(eid)
            # Use compact local evidence only as a variation signal; never as a
            # canonical human-facing PN.
            token_variants[token].add(alnum(pn) if pn and not _pn_placeholder(pn) else alnum(text)[:96])

    numeric_cores: List[Tuple[str, int]] = []
    for token, evs in token_events.items():
        exact_support = len(exact_events.get(token, set()))
        if len(evs) >= PRODUCT_PART_MIN_REPAIRS and (len(token_variants[token]) >= 2 or exact_support >= 2):
    ...
    for r in rows:
        ...
        text = compact(r.get("text"))
        combined = " ".join(x for x in (raw_pn if not _pn_placeholder(raw_pn) else "", text) if x)

        # Passive values are legitimate technician-facing identities even when
        # stored only in the replacement text.
        ...
        if cap:
            ...
            can = alnum(combined)
            # Prefer the strongest corpus-supported numeric core found anywhere
            # in this explicit replacement row.
            for token, support in numeric_cores:
                if token in can:
                    core, core_kind = token, "structured_numeric_core"
                    break

            if core is None and raw_pn and not _pn_placeholder(raw_pn):
                # Allow rare OCR mutation to join only a strongly-supported
                # four-digit core, and only from an actual PN field.
                ...
                for token, support in numeric_cores:
                    ...
                    if any(len(n) == 4 and _one_digit_apart(n, token) for n in rare_nums):
                        core, core_kind = token, "structured_numeric_core_fuzzy"
                        break

            if core is None and raw_pn and not _pn_placeholder(raw_pn):
                core, core_kind = raw_pn, "exact_pn"

        # No usable PN/core in this explicit parts_replaced row. Preserve it in
        # the underlying event/replacement corpus, but do not invent a simple
        # technician-facing part identity.
        if not core:
            continue
    ...
    # HARD 80/20 RULE: no expert/user-provided repair-kit projection is permitted.
    out: List[Dict[str, Any]] = []
    for key, g in groups.items():
        ...
        out.append({
            ...
            "item_key": f"structured_core_product_part:{key}",
            "primary_value": label,
            "title": label,
            "payload": {
                "pn": label,
                ...
                "core_kind": g["kind"],
                "source_policy": "explicit_structured_parts_replaced_only",
                "evidence_rows": g["evidence_rows"],
...
def command_plan(args) -> int:
    ...
    print("v1.5.8: structured parts_replaced aggregation + corpus-only component-core resolver + minimal product view")
    print("Search/database schema:          v1.5.3 full-corpus DB reused; v1.5.8 resolves product + component cores from structured parts_replaced evidence at query time")
