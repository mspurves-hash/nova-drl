#!/usr/bin/env python3
"""
Nova DRL Parts Replaced Fusion v1.5.3.1
=====================================

Extracts conservative part candidates from HUMAN-APPROVED repair actions that
have already passed through the v1.5.2.4 terminology layer.
...
- Does not modify v1.5.1 or v1.5.2.4 artifacts.
...
SERVICE_PATTERNS = [
    ...
    re.compile(r"\bresurfac(?:e|ed|ing)?\b", re.IGNORECASE),
    re.compile(r"\bvacuum(?:ed|ing)?\b", re.IGNORECASE),
]
...
def parse_quantity_details(value, start, end):
    """Parse only explicit quantities from a human-approved action.

    Priority is given to an explicit total such as ``(9 total)`` because it
    represents the action-level part quantity. Legacy xN / N x / qty forms and
    a numeric prefix remain supported. Nothing is inferred from axis count.
    """
    left = value[max(0, start - 24):start]
    right = value[end:min(len(value), end + 220)]
    combined = "{} {}".format(left, right)

    total_patterns = [
        r"\(\s*(\d{1,3})\s+total\s*\)",
        r"\btotal\s*[:=]?\s*(\d{1,3})\b",
    ]
    for pattern in total_patterns:
        match = re.search(pattern, right, flags=re.IGNORECASE)
        if match:
            return {
                "quantity": int(match.group(1)),
                "quantity_raw": match.group(0),
                "quantity_source": "explicit_total",
            }

    patterns = [
        (r"\bx\s*(\d{1,3})\b", "explicit_x_quantity"),
        (r"\b(\d{1,3})\s*x\b", "explicit_x_quantity"),
        (r"\bqty\.?\s*[:=]?\s*(\d{1,3})\b", "explicit_qty_label"),
        (r"\bquantity\s*[:=]?\s*(\d{1,3})\b", "explicit_quantity_label"),
    ]
    ...
    # Conservative numeric prefix, such as "2 bearings" or
    # "Installed 2 Blue Schmoo's".
    prefix = re.search(r"(\d{1,3})\s*$", left)
    if prefix:
        return {
            "quantity": int(prefix.group(1)),
            "quantity_raw": prefix.group(0),
            "quantity_source": "numeric_prefix",
        }

    return {
        "quantity": None,
        "quantity_raw": None,
        "quantity_source": "not_established",
    }


def parse_quantity(value, start, end):
    """Backward-compatible two-value quantity interface."""
    details = parse_quantity_details(value, start, end)
    return details["quantity"], details["quantity_raw"]


def parse_quantity_breakdown(value, mention_end, total_quantity=None):
    """Capture explicit ``N for context`` distributions after a part mention.

    The breakdown is evidence only. A total is marked verified only when an
    independently explicit total was also written in the approved action and
    the arithmetic agrees.
    """
    tail = value[mention_end:]
    pattern = re.compile(
        r"(?P<quantity>\d{1,3})\s+for\s+"
        r"(?P<context>.*?)"
        r"(?=(?:,\s*(?:and\s+)?|\s+and\s+)\d{1,3}\s+for|"
        r"\s*\(\s*\d{1,3}\s+total\s*\)|$)",
        flags=re.IGNORECASE,
    )
    breakdown = []
    ...
    breakdown_sum = sum(row["quantity"] for row in breakdown) if breakdown else None
    verified = bool(
        breakdown
        and total_quantity is not None
        and breakdown_sum == total_quantity
    )
    return breakdown, breakdown_sum, verified


def parse_identified_part_number(value, mention_start):
    """Return a conservative adjacent alphanumeric part-number token.

    The token must appear immediately before the part mention area, contain at
    least one letter and one digit, and be at least three characters long.
    This uses only the HUMAN-APPROVED action text and never raw OCR.
    """
    left = value[max(0, mention_start - 56):mention_start]
    tokens = list(re.finditer(r"[A-Za-z0-9][A-Za-z0-9._/-]{2,31}", left))
...
def classify_mention(action_value, mention):
    ...
    quantity_details = parse_quantity_details(
        action_value,
        mention["start"],
        mention["end"],
    )
    quantity = quantity_details["quantity"]
    breakdown, breakdown_sum, breakdown_verified = parse_quantity_breakdown(
        action_value,
        mention["end"],
        total_quantity=quantity,
    )
    identified_part_number = parse_identified_part_number(
        action_value,
        mention["start"],
    )
    ...
    return {
        **mention,
        ...
        "quantity": quantity,
        "quantity_raw": quantity_details["quantity_raw"],
        "quantity_source": quantity_details["quantity_source"],
        "quantity_breakdown": breakdown,
        "quantity_breakdown_sum": breakdown_sum,
        "quantity_breakdown_verified": breakdown_verified,
        "identified_part_number": identified_part_number,
...
def record_part_decision(
    ...
    record = {
        ...
        "candidate_quantity_source_at_review": item.get("quantity_source"),
        "identified_part_number": item.get("identified_part_number"),
        "quantity_breakdown": item.get("quantity_breakdown") or [],
        "quantity_breakdown_verified": bool(item.get("quantity_breakdown_verified")),
...
def approved_parts(parts_field):
    ...
    for item in parts_field.get("candidates", []):
        ...
        output.append({
            ...
            "quantity_breakdown": copy.deepcopy(item.get("quantity_breakdown") or []),
            "quantity_breakdown_sum": item.get("quantity_breakdown_sum"),
            "quantity_breakdown_verified": bool(item.get("quantity_breakdown_verified")),
            "source_action_id": item.get("source_action_id"),
            "source_action_number": item.get("source_action_number"),
...
def build_output(source_data, source_path, part_terms, decisions):
    ...
    output["parts_replaced_policy"] = {
        ...
        "explicit_total_quantity_supported": True,
        "quantity_breakdown_preserved": True,
        "part_number_from_approved_action_only": True,
        "service_verbs_include_resurfaced_and_vacuumed": True,
...
def render_review(output):
    ...
    for item in field.get("candidates", []):
        ...
        lines += [
            ...
            "  Quantity source: {}".format(item.get("quantity_source") or "not_established"),
            "  Identified part number: {}".format(item.get("identified_part_number") or "None"),
            ...
            "  Quantity breakdown: {}".format(
                item.get("quantity_breakdown") or "None"
            ),
            "  Quantity breakdown sum: {}".format(
                item.get("quantity_breakdown_sum")
                if item.get("quantity_breakdown_sum") is not None
                else "None"
            ),
            "  Quantity breakdown verified: {}".format(
                "YES" if item.get("quantity_breakdown_verified") else "NO"
            ),
