...
def default_rules_path():
    return (
        ...
        / "config"
        / "testing_final_result_rules_v1_5_5_4.json"
    )
...
def build_review(analyses, rules, decisions):
    ...
    return {
        ...
        "source_policy": {
            ...
            "globally_ignored_testing_duration_used_as_context": False,
            "globally_ignored_testing_duration_qdrant_eligible": False,
        },
        "qdrant_entry_created": False,
    }
...
def analyze_sources(
    ...
    analyses = []
    cache_dir = (
        Path(output_dir) / "page_analysis_cache_v1_5_5_4"
    )
    cache_dir.mkdir(parents=True, exist_ok=True)
...
# ============================================================================
# v1.5.5.4 ANCHOR-AWARE FORM ASSOCIATION OVERRIDES
# ============================================================================

NON_TEST_EVENT_HEADER_RE = re.compile(
    r"^\s*(?:customer(?:\s+(?:problem|complaint|symptom))?|"
    r"customer\s+problem\s*/\s*symptom\s+description|"
    r"problem\s*/\s*symptom\s+description|"
    r"failure\s+(?:description|analysis)|"
    r"rma(?:\s+number)?|serial(?:\s+number)?|"
    r"repair\s+technicians?|repair\s+type|product\s+type|"
    r"date|log(?:\s+number)?|model|oem)\s*$",
    re.IGNORECASE,
)

GENERIC_RESULT_BASIS_RE = re.compile(
    r"(?:\s/\s|field\s+label|literal\s+associated|"
    r"final\s+result\s*/|test\s+result\s*/|overall\s+result\s*/)",
    re.IGNORECASE,
)

SHIPPING_DURATION_MARK_RE = re.compile(
    r"(?:^\s*\d+(?:\.\d+)?\+?\s*$|\b\d+(?:\.\d+)?\+?\s*(?:hours?|hrs?)\b|"
    r"\b(?:hours?|hrs?)\b)",
    re.IGNORECASE,
)

ANCHOR_SELECTION_STATUSES = {"selected", "not_selected", "ambiguous"}
ANCHOR_MARK_TYPES = {
    "checkmark", "x_mark", "initials", "handwritten_value", "circle", "other"
}


def default_anchor_profiles_path():
    return (
        Path(__file__).resolve().parents[1]
        / "config"
        / "testing_anchor_profiles_v1_5_5_4.json"
    )


def load_anchor_profiles(path):
    data = read_json(path)
    if not isinstance(data.get("profiles"), dict):
        raise ValueError("Anchor profiles must contain a profiles object.")
    return data
...
def _tesseract_tsv_words(image_path, timeout=60):
    """Return OCR word boxes from Tesseract TSV. Safe failure -> status+empty list."""
    image_path = Path(image_path)
    if not image_path.exists():
        return {"status": "image_missing", "words": [], "warning": str(image_path)}
    ...
    for psm in (6, 11):
        try:
            ...
            candidate = {"status": "tesseract_error", "words": [], "warning": str(exc)}
            if best is None:
                best = candidate
            continue
        ...
        if proc.returncode != 0:
            candidate = {
                ...
                "words": [],
                "warning": proc.stderr.strip()[:1000],
            }
            if best is None:
                best = candidate
            continue
        ...
        try:
            ...
            candidate = {"status": "tsv_parse_error", "words": [], "warning": str(exc)}
            if best is None:
                best = candidate
            continue
        ...
        if best is None or len(words) > len(best.get("words", [])):
            best = candidate

    return best or {"status": "tesseract_error", "words": [], "warning": "no OCR result"}


def _group_tsv_lines(words):
    ...
    for key, rows in grouped.items():
        rows = sorted(rows, key=lambda x: (x.get("word_num", 0), x.get("left", 0)))
...
def find_anchor_bbox(words, aliases, minimum_score=0.72):
    """Find a printed label by OCR words; no absolute form coordinates used."""
    aliases = [a for a in aliases if _norm_tokens(a)]
    best = None

    for line_key, line_words in _group_tsv_lines(words):
        normalized = []
        original_refs = []
        for word in line_words:
            toks = _norm_tokens(word.get("text"))
            for tok in toks:
                normalized.append(tok)
                original_refs.append(word)

        if not normalized:
            continue

        for alias in aliases:
            target_tokens = _norm_tokens(alias)
            target = " ".join(target_tokens)
            n = len(target_tokens)
            for start in range(len(normalized)):
                for length in range(max(1, n - 1), min(len(normalized) - start, n + 2) + 1):
                    candidate_tokens = normalized[start:start + length]
                    ...
                    if score < minimum_score:
                        continue
                    refs = original_refs[start:start + length]
                    unique_refs = []
                    seen = set()
                    for ref in refs:
                        ident = (ref["left"], ref["top"], ref["width"], ref["height"], ref["text"])
                        if ident in seen:
                            continue
                        seen.add(ident)
                    ...
                    record = {
                        "alias": alias,
                        "score": round(score, 4),
                        "matched_text": " ".join(r["text"] for r in unique_refs),
                        "bbox": _bbox_for_words(unique_refs),
...
GLOBAL_IGNORE_PATTERNS_1554 = [
    re.compile(r"48\+", re.IGNORECASE),
    re.compile(r"48\+\s*(?:hours?|hrs?)", re.IGNORECASE),
    re.compile(r"hours?\s+in\s+final\s+testing", re.IGNORECASE),
    re.compile(r"final\s+testing\s+duration", re.IGNORECASE),
]

GLOBAL_IGNORE_LABEL_PATTERNS_1554 = [
    re.compile(r"hours?\s+in\s+final\s+testing", re.IGNORECASE),
    re.compile(r"final\s+testing\s+duration", re.IGNORECASE),
]

ANCHOR_SCHEMA_PLACEHOLDER_MARKS_1554 = {
    "handwritten_value",
    "handwritten value",
    "initials",
    "event_mark",
...
ANCHOR_LITERAL_SELECTION_MARKS_1554 = {
    "x", "x mark", "x_mark", "checkmark", "checked box",
    "handwritten checkmark", "handwritten_checkmark",
    "circle", "circled", "yes", "selected", "✓", "✔", "☒",
}

INITIALS_RE_1554 = re.compile(r"^[A-Za-z]{1,4}(?:/[A-Za-z]{1,4})?$")


def _globally_ignored_text_1554(value):
    text_value = _clean_text(value)
    if not text_value:
        return False
    return any(pattern.search(text_value) for pattern in GLOBAL_IGNORE_PATTERNS_1554)


def _globally_ignored_label_1554(value):
    text_value = _clean_text(value)
    if not text_value:
        return False
    return any(pattern.search(text_value) for pattern in GLOBAL_IGNORE_LABEL_PATTERNS_1554)


def _item_has_global_ignore_1554(item):
    if not isinstance(item, dict):
        return False
    for key in (
        "step_label", "event_mark", "recorded_value", "value", "basis_label",
        "technician_initials", "date", "selected_result"
    ):
        if _globally_ignored_text_1554(item.get(key)) or _globally_ignored_label_1554(item.get(key)):
            return True
    return False


def _ignored_ocr_word_boxes_1554(words):
    boxes = []
    for word in words or []:
        raw = _clean_text(word.get("text"))
        if not raw:
            continue
        # The human-directed ignore is anchored by the duration value itself.
        if not re.search(r"48\+", raw, re.IGNORECASE):
            continue
        boxes.append([
            int(word.get("left") or 0),
            int(word.get("top") or 0),
...
def _relative_anchor_crop(image_path, bbox, field, output_path, ocr_words=None):
    """
    Relative anchor crop only. No absolute form coordinates.

    v1.5.5.4 additionally masks the human-directed globally ignored final-testing
    duration value before local Final O.K. vision so it cannot contaminate mark
    association.
    """
    ...
    crop_cfg = field.get("crop", {}) or {}
    left_h = float(crop_cfg.get("left_height_multiplier", 5.0))
    right_h = float(crop_cfg.get("right_height_multiplier", 5.0))
    top_h = float(crop_cfg.get("top_height_multiplier", 1.5))
    bottom_h = float(crop_cfg.get("bottom_height_multiplier", 1.5))

    left = max(0, int(x - left_h * h))
    right = min(width, int(x + w + right_h * h))
    top = max(0, int(y - top_h * h))
    bottom = min(height, int(y + h + bottom_h * h))
    if right <= left or bottom <= top:
        raise ValueError("Anchor crop bounds are empty.")

    crop = image.crop((left, top, right, bottom))
    ignored_boxes = []
    if field.get("global_ignore_policy") == "ignore_final_testing_duration":
        draw = ImageDraw.Draw(crop)
        for bx, by, bw, bh, raw in _ignored_ocr_word_boxes_1554(ocr_words):
            # Only mask ignored OCR evidence that actually intersects this crop.
            abs_l = bx
            abs_t = by
            abs_r = bx + bw
            abs_b = by + bh
            if abs_r < left or abs_l > right or abs_b < top or abs_t > bottom:
                continue

            # Expand around the duration value enough to eliminate the nearby
            # handwritten duration token without covering the target label.
            pad_x = max(int(max(bh, 12) * 1.35), 10)
            pad_y = max(int(max(bh, 12) * 0.55), 6)
            ml = max(left, abs_l - pad_x)
            mt = max(top, abs_t - pad_y)
            mr = min(right, abs_r + pad_x)
            mb = min(bottom, abs_b + pad_y)

            rel_box = [ml - left, mt - top, mr - left, mb - top]
            draw.rectangle(tuple(rel_box), fill="white")
            ignored_boxes.append({
                "raw_ocr": raw,
                "absolute_box": [ml, mt, mr, mb],
                "crop_relative_box": rel_box,
                "policy": "ignore_final_testing_duration",
                "accepted_as_repair_fact": False,
                "qdrant_entry_created": False,
            })
    ...
    return {
        ...
        "globally_ignored_regions_masked": len(ignored_boxes),
        "ignored_regions": ignored_boxes,
    }


def _anchor_field_prompt(profile_name, field):
    label = field.get("label")
    special = ""
    if profile_name == "DRL_TRAVELER_FINAL_TEST":
        special += (
            "\nThis target belongs to a mutually exclusive group. Inspect ONLY the mark "
            "immediately associated with the TARGET LABEL. Marks beside other options do "
            "not select this target."
        )
    if profile_name == "DRL_TRAVELER_SHIPPING_FINAL_OK":
        special += (
            "\nA neighboring testing-duration field is globally ignored by human direction "
            "and may be masked in this crop. Do not use any neighboring duration value, "
            "duration text, or masked area as evidence for the TARGET LABEL."
        )

    return """You are examining a SMALL CROP around one known Direct Repair Laboratories form label.

FORM PROFILE: {profile}
TARGET LABEL: {label}

Determine only whether the event-specific mark immediately associated with the
TARGET LABEL selects/confirms that target.

RULES:
1. Printed checkbox outlines or printed text are not selections.
2. Ignore marks that belong to neighboring labels or rows.
3. Do not infer selection from document context.
4. If the target's own box/field is clearly marked with X/check/circle/initials,
   return selected.
5. If the target's own field is visibly blank, return not_selected.
6. If association is uncertain, return ambiguous.
7. event_mark must be the literal visible event-specific ink/mark associated
   with this target. Never return a schema placeholder such as
   "handwritten_value", "initials", or "event_mark".
8. If mark_type is initials, technician_initials must contain the literal
   alphabetic initials actually visible in the target field.
9. Numeric values cannot be technician initials.
10. Return JSON only.{special}
...
{{
  ...
  "event_mark": null,
  "mark_type": "checkmark|x_mark|initials|handwritten_value|circle|other",
  "technician_initials": null,
  "date": null,
  "confidence": "high|medium|low",
  "notes": null
}}
""".format(profile=profile_name, label=label, special=special)

def _anchor_cache_signature(source, field, anchor, model, no_vision):
    image_path = Path(source.get("image_path") or "")
    stat_info = None
    if image_path.exists():
        stat = image_path.stat()
        stat_info = {"size": stat.st_size, "mtime_ns": stat.st_mtime_ns}
    payload = {
        "cache_schema": "1.5.5.4-anchor",
        "source_path": source.get("source_path"),
        "image_path": source.get("image_path"),
        "source_document": source.get("source_document"),
        "profile": _form_profile(source),
        ...
        "model": model,
        "no_vision": bool(no_vision),
        "image_stat": stat_info,
        "prompt_sha256": hashlib.sha256(
            _anchor_field_prompt(_form_profile(source), field).encode("utf-8")
        ).hexdigest(),
    }
...
def _normalize_anchor_vision(parsed, profile_name=None, field=None):
    parsed = parsed if isinstance(parsed, dict) else {}
    field = field or {}

    status = _clean_text(parsed.get("selection_status")).lower()
    if status not in ANCHOR_SELECTION_STATUSES:
        status = "ambiguous"

    mark_type = _clean_text(parsed.get("mark_type")).lower()
    if mark_type not in ANCHOR_MARK_TYPES:
        mark_type = "other"

    mark = parsed.get("event_mark")
    mark = _clean_text(mark) if mark is not None else None
    if mark == "":
        mark = None

    initials = parsed.get("technician_initials")
    initials = _clean_text(initials) if initials is not None else None
    if initials == "":
        initials = None

    date = parsed.get("date")
    date = _clean_text(date) if date is not None else None
    if date == "":
        date = None

    confidence = normalize_confidence(parsed.get("confidence"))
    reasons = []
    ignored_contamination = False

    # Human-directed global ignore: the final-testing-duration field cannot be
    # used as event mark, initials, date, or association context.
    for key, value in (("event_mark", mark), ("technician_initials", initials), ("date", date)):
        if _globally_ignored_text_1554(value) or _globally_ignored_label_1554(value):
            ignored_contamination = True
            reasons.append("globally_ignored_testing_duration_contaminated_{}".format(key))
            if key == "event_mark":
                mark = None
            elif key == "technician_initials":
                initials = None
            else:
                date = None

    if mark and mark.casefold() in ANCHOR_SCHEMA_PLACEHOLDER_MARKS_1554:
        reasons.append("schema_placeholder_event_mark_rejected")
        mark = None

    if mark_type == "initials":
        literal_initials = None
        if initials and INITIALS_RE_1554.fullmatch(initials):
            literal_initials = initials
        elif mark and INITIALS_RE_1554.fullmatch(mark):
            literal_initials = mark

        if literal_initials:
            initials = literal_initials
            mark = literal_initials
        else:
            reasons.append("invalid_or_nonliteral_technician_initials")
            initials = None
            if status == "selected":
                status = "ambiguous"

    elif mark_type in {"checkmark", "x_mark", "circle"}:
        if mark and mark.casefold() not in ANCHOR_LITERAL_SELECTION_MARKS_1554:
            # Keep literal marks such as a drawn symbol, but do not accept a
            # neighboring sentence/value as the target selection.
            if len(mark) > 16 or PRINTED_INSTRUCTION_MARK_RE.search(mark):
                reasons.append("selection_mark_not_compact_or_literal")
                mark = None
                if status == "selected":
                    status = "ambiguous"

    elif mark_type == "handwritten_value":
        if not mark:
            reasons.append("handwritten_value_missing_literal_value")
            if status == "selected":
                status = "ambiguous"

    if ignored_contamination and status == "selected":
        status = "ambiguous"

    if status == "selected" and not mark:
        reasons.append("selected_without_valid_literal_event_mark")
        status = "ambiguous"

    return {
        "target_label_seen": bool(parsed.get("target_label_seen")),
        ...
        "global_ignore_contamination_detected": ignored_contamination,
        "global_ignore_policy": (
            "ignore_final_testing_duration"
            if profile_name == "DRL_TRAVELER_SHIPPING_FINAL_OK"
            else None
        ),
    }

def analyze_anchor_fields(
    source_images,
    output_dir,
    model,
    timeout,
    ...
    """Targeted label-anchor verification for fixed traveler final fields."""
    results = []
    cache_dir = Path(output_dir) / "field_verification_cache_v1_5_5_4"
    cache_dir.mkdir(parents=True, exist_ok=True)
    model_available = ollama_model_exists(model) if not no_vision else False

    for source in source_images:
        profile_name = _form_profile(source)
        profile = (anchor_profiles.get("profiles", {}) or {}).get(profile_name)
        if not profile:
            continue
        ...
        ocr = _tesseract_tsv_words(source.get("image_path"), timeout=min(timeout, 60))
        words = ocr.get("words", [])

        for field in fields:
            aliases = [field.get("label")] + list(field.get("aliases", []) or [])
            anchor_match = find_anchor_bbox(words, aliases) if words else None
            ...
            cache_path = cache_dir / (record_key + ".json")

            base_record = {
                "verification_id": record_key,
                "fusion_version": VERSION,
                "profile": profile_name,
                "field_id": field.get("field_id"),
                "target_label": field.get("label"),
                "canonical_value": field.get("canonical_value"),
                "canonical_result": field.get("canonical_result"),
                "mutually_exclusive_group": field.get("mutually_exclusive_group"),
                "source": copy.deepcopy(source),
                "anchor_ocr_status": ocr.get("status"),
                "anchor_ocr_warning": ocr.get("warning"),
                "anchor_match": anchor_match,
                "anchor_status": "found" if anchor_match else "not_found",
                "vision_model": model,
                "vision_status": "not_run",
                "raw_vision_response": "",
                "verification": {
            ...
            if not anchor_match:
                base_record["vision_status"] = "anchor_not_found"
                write_json(cache_path, base_record)
                results.append(base_record)
                continue

            signature, signature_inputs = _anchor_cache_signature(
                source, field, anchor_match, model, no_vision
            )
            if cache_path.exists() and not refresh:
                try:
                    cached = read_json(cache_path)
                    ...
                    cached = None
                if isinstance(cached, dict) and cached.get("cache_signature") == signature:
                    cached["cache_status"] = "reused"
                    results.append(cached)
                    continue

            base_record["cache_signature"] = signature
            base_record["cache_signature_inputs"] = signature_inputs
            if cache_path.exists():
                base_record["cache_status"] = "refreshed" if refresh else "invalidated"

            crop_path = cache_dir / "anchor_crops" / (record_key + ".png")
            try:
                crop_info = _relative_anchor_crop(
                    source.get("image_path"), anchor_match["bbox"], field, crop_path, ocr_words=words
                )
                base_record["anchor_crop"] = crop_info
            except Exception as exc:
                base_record["vision_status"] = "anchor_crop_error"
                base_record["warning"] = str(exc)
                write_json(cache_path, base_record)
                results.append(base_record)
                continue

            if no_vision:
                base_record["vision_status"] = "vision_disabled"
                write_json(cache_path, base_record)
                results.append(base_record)
                continue
            if not model_available:
                base_record["vision_status"] = "model_unavailable"
                base_record["warning"] = "Ollama model {} was not found.".format(model)
                write_json(cache_path, base_record)
                results.append(base_record)
                continue
            ...
            response = call_ollama_vision(
                model,
                _anchor_field_prompt(profile_name, field),
                crop_info["crop_path"],
                timeout,
            )
            base_record["raw_vision_response"] = response.get("response", "")
            base_record["vision_status"] = response.get("status")
            base_record["warning"] = response.get("warning")
            if response.get("status") == "ok":
                parsed = parse_json_object(response.get("response"))
                if parsed is None:
                    base_record["vision_status"] = "response_not_json"
                else:
                    base_record["verification"] = _normalize_anchor_vision(parsed, profile_name, field)
            write_json(cache_path, base_record)
            results.append(base_record)

    write_json(
        cache_dir / "field_verification_manifest.json",
        {
            "fusion_version": VERSION,
            "generated_at_utc": now_utc(),
            "record_count": len(results),
            "cache_status_counts": dict(collections.Counter(r.get("cache_status") for r in results)),
            "vision_status_counts": dict(collections.Counter(r.get("vision_status") for r in results)),
            "selection_status_counts": dict(collections.Counter((r.get("verification") or {}).get("selection_status") for r in results)),
            "qdrant_entry_created": False,
        },
    )
    return results
...
def _approved_complaint_texts(source_data):
    values = set(_approved_complaint_values(source_data))
    complaint = source_data.get("approved_fields", {}).get("customer_complaint")
    if isinstance(complaint, dict):
        value = _clean_text(complaint.get("value"))
        if value:
...
def _event_header_or_complaint_testing(step, mark, source_data):
    step_clean = _clean_text(step)
    mark_clean = _clean_text(mark)
    if NON_TEST_EVENT_HEADER_RE.match(step_clean):
        return True
    if CUSTOMER_PROBLEM_LABEL_RE.search(step_clean):
        return True
    complaints = _approved_complaint_texts(source_data or {})
    if mark_clean.casefold() in complaints and complaints:
        return True
    return False


def build_testing_candidates_hardened_1553(analyses, rules, source_data):
    # Start from v1.5.5.2 behavior but apply header/complaint authority gate first.
    filtered_analyses = []
    pre_rejections = []
    for analysis in analyses:
        clone = copy.deepcopy(analysis)
        parsed = clone.get("parsed_analysis", {})
        source = clone.get("source", {})
        kept = []
        for item in parsed.get("testing_items", []) or []:
            if _item_has_global_ignore_1554(item):
                pre_rejections.append(
                    _rejection_record(
                        "testing_performed",
                        source,
                        item,
                        "globally_ignored_testing_duration_reference",
                        clone.get("analysis_id"),
                    )
                )
            elif _event_header_or_complaint_testing(
                item.get("step_label"), item.get("event_mark"), source_data
            ):
                pre_rejections.append(
                    _rejection_record(
                        "testing_performed",
                        source,
                        item,
                        "event_header_or_customer_problem_not_testing",
                        clone.get("analysis_id"),
                    )
                )
            else:
                kept.append(item)
        parsed["testing_items"] = kept
        parsed["other_event_observations"] = [
            obs for obs in (parsed.get("other_event_observations", []) or [])
            if not _item_has_global_ignore_1554(obs)
            and not _globally_ignored_label_1554(obs.get("label"))
        ]
        clone["parsed_analysis"] = parsed
        filtered_analyses.append(clone)

    rows, rejected, routed = build_testing_candidates_hardened(
        filtered_analyses, rules
    )
    return rows, pre_rejections + rejected, routed


def _verified_template_basis(source, basis):
    basis = _clean_text(basis)
    ocr = _clean_text(source.get("template_ocr_text"))
    if not basis or not ocr:
        return False, 0.0
    if GENERIC_RESULT_BASIS_RE.search(basis):
        return False, 0.0
    if _clean_text(basis).casefold() in DOCUMENT_TITLE_BASIS:
        return False, 0.0
    bnorm = _norm_phrase(basis)
    onorm = _norm_phrase(ocr)
    if len(bnorm) < 5:
        return False, 0.0
    if bnorm in onorm:
        return True, 1.0

    # Compare against OCR line-like segments. Require high similarity because
    # this is an authority gate, not fuzzy content recovery.
    best = 0.0
    for segment in re.split(r"[\n\r|]+", str(source.get("template_ocr_text") or "")):
        snorm = _norm_phrase(segment)
        if not snorm:
            continue
        score = difflib.SequenceMatcher(None, bnorm, snorm).ratio()
        best = max(best, score)
    return best >= 0.88, round(best, 4)


def _supporting_final_candidates_1553(analyses, source_data, rules):
    max_mark_length = int(rules.get("strict_schema", {}).get("maximum_event_mark_length", 96))
    complaint_values = _approved_complaint_values(source_data)
    rows, rejected = [], []
    ...
    for analysis in analyses:
        ...
        if _is_traveler_final_crop(source):
            continue
        analysis_id = analysis.get("analysis_id")
        for item in analysis.get("parsed_analysis", {}).get("final_result_items", []) or []:
            value = _clean_text(item.get("value"))
            basis = _clean_text(item.get("basis_label"))
            mark = _clean_text(item.get("event_mark"))
            if _item_has_global_ignore_1554(item):
                rejected.append(_rejection_record(
                    "final_result", source, item,
                    "globally_ignored_testing_duration_reference", analysis_id
                )); continue
            if not value:
                rejected.append(_rejection_record("final_result", source, item, "blank_final_value", analysis_id)); continue
            if value.casefold() in complaint_values or CUSTOMER_PROBLEM_LABEL_RE.search(basis):
                rejected.append(_rejection_record("final_result", source, item, "customer_problem_field_cannot_be_final_result", analysis_id)); continue
            if PAGE_HEADER_RE.search(value):
                rejected.append(_rejection_record("final_result", source, item, "page_header_or_document_title_not_final_result", analysis_id)); continue
            ...
            anchor_ok, anchor_score = _verified_template_basis(source, basis)
            if not anchor_ok:
                enriched = copy.deepcopy(item)
                enriched["template_anchor_score"] = anchor_score
                rejected.append(_rejection_record("final_result", source, enriched, "basis_label_not_verified_in_template_ocr", analysis_id)); continue

            result = _clean_text(item.get("result")).lower()
            selected_result = _clean_text(item.get("selected_result")).lower() or None
            if result not in FINAL_RESULT_ENUMS_1552:
                rejected.append(_rejection_record("final_result", source, item, "invalid_final_result_enum", analysis_id)); continue
            canonical_result = selected_result if selected_result in {"pass", "fail"} else result
            valid_mark, mark_reason = _mark_validation(mark, None, max_mark_length)
            if not valid_mark:
                rejected.append(_rejection_record("final_result", source, item, mark_reason, analysis_id)); continue

            rows.append({
                "candidate_id": stable_id("final1553-supporting", analysis_id, value, basis, mark, canonical_result),
                "final_number": None,
                "candidate_type": "final_result_candidate",
                "form_profile": _form_profile(source),
                "value": value,
                "source_value": value,
                "basis_label": basis,
                "basis_anchor_verified": True,
                "basis_anchor_score": anchor_score,
                "event_mark": mark,
                ...
                "source": copy.deepcopy(source),
                "source_analysis_id": analysis_id,
                "conflict_flags": _final_conflict_flags(value, source_data),
                "conflict_review_required": False,
                "approval_requires_conflict_acknowledgement": False,
                "human_review": {"status":"pending","reviewer":None,"reviewed_at_utc":None,"approved_value":None,"note":None},
                "accepted_as_human_reviewed_final_result": False,
                "qdrant": {"eligible_for_future_ingestion":False,"entry_created":False,"reason":"pending_human_review"},
            })

    # Apply conflict state after all fields are built.
    for row in rows:
        row["conflict_review_required"] = bool(row.get("conflict_flags"))
        row["approval_requires_conflict_acknowledgement"] = bool(row.get("conflict_flags"))
        if row.get("conflict_flags"):
            row["qdrant"]["reason"] = "conflict_review_required"
    return rows, rejected


def _traveler_final_candidates_from_anchor_verifications(field_verifications, source_data):
    rows, rejected, ambiguities = [], [], []
    grouped = collections.defaultdict(list)
    singles = []

    for record in field_verifications or []:
        verification = record.get("verification", {}) or {}
        group = record.get("mutually_exclusive_group")
        if group:
            grouped[(str(record.get("source", {}).get("source_path")), group)].append(record)
        else:
            singles.append(record)

    # Mutually exclusive traveler disposition groups: require exactly one
    # selected and every sibling unambiguously not selected.
    for (source_path, group_name), records in grouped.items():
        selected = [r for r in records if (r.get("verification") or {}).get("selection_status") == "selected"]
        ambiguous = [r for r in records if (r.get("verification") or {}).get("selection_status") == "ambiguous"]
        not_selected = [r for r in records if (r.get("verification") or {}).get("selection_status") == "not_selected"]
        complete = len(records) == len(selected) + len(ambiguous) + len(not_selected)

        if len(selected) != 1 or ambiguous or not complete:
            ambiguities.append({
                "ambiguity_id": stable_id("anchor-group", source_path, group_name, [(r.get("field_id"),(r.get("verification") or {}).get("selection_status")) for r in records]),
                "source_path": source_path,
                "group": group_name,
                "status": "ambiguous_group",
                "selected_field_ids": [r.get("field_id") for r in selected],
                "ambiguous_field_ids": [r.get("field_id") for r in ambiguous],
                "all_field_statuses": {r.get("field_id"):(r.get("verification") or {}).get("selection_status") for r in records},
                "candidate_created": False,
                "reason": "mutually_exclusive_group_not_uniquely_resolved",
            })
            continue

        record = selected[0]
        v = record.get("verification", {})
        mark = _clean_text(v.get("event_mark"))
        if not mark:
            rejected.append({
                "field": "final_result",
                "reason": "anchor_selected_without_literal_event_mark",
                "field_verification": copy.deepcopy(record),
                "accepted_as_repair_fact": False,
                "qdrant_entry_created": False,
            })
            continue
        ...
        result = record.get("canonical_result")
        flags = _final_conflict_flags(value, source_data)
        rows.append({
            "candidate_id": stable_id("final1553-anchor", record.get("verification_id"), value, mark, result),
            "final_number": None,
            "candidate_type": "final_result_candidate",
            "form_profile": record.get("profile"),
            "value": value,
            "source_value": record.get("target_label"),
            "basis_label": record.get("target_label"),
            "event_mark": mark,
            "raw_model_result": v.get("selection_status"),
            "result": result,
            "semantic_role": "final_disposition",
            "association_basis": "ocr_label_anchor_plus_local_vision",
            "selected_result": result,
            "confidence": v.get("confidence"),
            "source": copy.deepcopy(record.get("source", {})),
            "source_analysis_id": None,
            "field_verification_id": record.get("verification_id"),
            "anchor_match": copy.deepcopy(record.get("anchor_match")),
            "anchor_crop": copy.deepcopy(record.get("anchor_crop")),
            "conflict_flags": flags,
            "conflict_review_required": bool(flags),
            "approval_requires_conflict_acknowledgement": bool(flags),
            "human_review": {"status":"pending","reviewer":None,"reviewed_at_utc":None,"approved_value":None,"note":None},
            "accepted_as_human_reviewed_final_result": False,
            "qdrant": {"eligible_for_future_ingestion":False,"entry_created":False,"reason":"conflict_review_required" if flags else "pending_human_review"},
        })

    # Non-exclusive known fields such as shipping Final O.K.
    for record in singles:
        v = record.get("verification", {}) or {}
        if v.get("selection_status") != "selected":
            continue
        mark = _clean_text(v.get("event_mark"))
        initials = _clean_text(v.get("technician_initials"))
        date = _clean_text(v.get("date"))
        if (
            _globally_ignored_text_1554(mark)
            or _globally_ignored_text_1554(initials)
            or _globally_ignored_text_1554(date)
            or v.get("global_ignore_contamination_detected")
        ):
            rejected.append({
                "field": "final_result",
                "reason": "globally_ignored_testing_duration_contaminated_final_ok",
                "field_verification": copy.deepcopy(record),
                "accepted_as_repair_fact": False,
                "qdrant_entry_created": False,
            })
        ...
        if not mark:
            ...
            continue
        if record.get("profile") == "DRL_TRAVELER_SHIPPING_FINAL_OK" and SHIPPING_DURATION_MARK_RE.search(mark):
            rejected.append({
                "field": "final_result",
                "reason": "shipping_final_ok_mark_is_testing_duration_not_final_ok_mark",
                "field_verification": copy.deepcopy(record),
                "accepted_as_repair_fact": False,
                "qdrant_entry_created": False,
            })
    ...
    return rows, rejected, ambiguities


def build_final_candidates_hardened_1553(analyses, source_data, rules, field_verifications):
    supporting, support_rejections = _supporting_final_candidates_1553(
        analyses, source_data, rules
    )
    traveler, traveler_rejections, ambiguities = _traveler_final_candidates_from_anchor_verifications(
        field_verifications, source_data
    )
    rows = supporting + traveler
    ...
    for row in rows:
        key = (
            row.get("value", "").casefold(),
            row.get("event_mark", "").casefold(),
            str(row.get("source", {}).get("source_path")),
            row.get("source", {}).get("page_number"),
            row.get("field_verification_id"),
        )
        if key in seen:
            continue
        seen.add(key)
    ...
    for index, row in enumerate(unique, start=1):
        row["final_number"] = index
    return unique, support_rejections + traveler_rejections, [], ambiguities


def build_review(analyses, rules, decisions, source_data=None, field_verifications=None):
    source_data = source_data or {}
    field_verifications = field_verifications or []

    testing, testing_rejections, testing_routed = build_testing_candidates_hardened_1553(
        analyses, rules, source_data
    )
    finals, final_rejections, final_routed, anchor_ambiguities = build_final_candidates_hardened_1553(
        analyses, source_data, rules, field_verifications
    )
    apply_decisions(testing, finals, decisions)
    ...
    for analysis in analyses:
        ...
        for observation in parsed.get("other_event_observations", []):
            label = _clean_text(observation.get("label"))
            value = _clean_text(observation.get("value"))
            if _globally_ignored_text_1554(label) or _globally_ignored_text_1554(value) or _globally_ignored_label_1554(label):
                continue
            if label or value:
                model_observations.append(_observation_record(
                    source,
                    label,
                    value,
                    _observation_category(label),
    ...
    approved_tests = sum(r["human_review"]["status"] == "approved" for r in testing)
    approved_finals = sum(r["human_review"]["status"] == "approved" for r in finals)
    conflict_count = sum(bool(r.get("conflict_flags")) for r in finals)
    ...
    return {
        ...
        "testing": {
            "candidate_count": len(testing),
            "approved_count": approved_tests,
            "pending_count": sum(r["human_review"]["status"] == "pending" for r in testing),
            "candidates": testing,
        },
        "final_result": {
            "candidate_count": len(finals),
            "approved_count": approved_finals,
            "pending_count": sum(r["human_review"]["status"] == "pending" for r in finals),
            "conflict_candidate_count": conflict_count,
            "status": "human_approved" if approved_finals else ("candidate_pending_human_review" if finals else "not_established"),
            "candidates": finals,
        },
        "anchor_field_verification": {
            "verification_count": len(field_verifications),
            "anchor_found_count": sum(r.get("anchor_status") == "found" for r in field_verifications),
            "selected_count": sum((r.get("verification") or {}).get("selection_status") == "selected" for r in field_verifications),
            "not_selected_count": sum((r.get("verification") or {}).get("selection_status") == "not_selected" for r in field_verifications),
            "ambiguous_count": sum((r.get("verification") or {}).get("selection_status") == "ambiguous" for r in field_verifications),
            "cache_reused_count": sum(r.get("cache_status") == "reused" for r in field_verifications),
            "ambiguity_group_count": len(anchor_ambiguities),
            "ambiguity_groups": anchor_ambiguities,
            "verifications": field_verifications,
        },
        "hardening": {
            "testing_rejection_count": len(testing_rejections),
            "final_rejection_count": len(final_rejections),
            "routed_observation_count": len(routed_unique),
            "testing_rejections": testing_rejections,
            "final_rejections": final_rejections,
            ...
            "strict_schema_enforced": True,
            "source_aware_routing_enabled": True,
            "customer_complaint_testing_block_enabled": True,
            "supporting_result_basis_must_be_template_anchored": True,
            "traveler_final_fields_use_ocr_anchor_plus_local_vision": True,
            "mutually_exclusive_group_requires_unique_resolution": True,
            "shipping_duration_cannot_mark_final_ok": True,
            "global_testing_duration_ignore_enabled": True,
            "global_ignore_rejection_count": sum(
                1 for r in (testing_rejections + final_rejections)
                if "globally_ignored_testing_duration" in str(r.get("reason", ""))
            ),
        },
        "printed_template_only_observations": printed_only,
        "uncertain_mark_observations": uncertain,
        "source_policy": {
            "printed_template_is_not_completion_evidence": True,
            "document_title_is_not_final_result": True,
            "customer_complaint_is_not_testing": True,
            "customer_complaint_is_not_final_result": True,
            "supporting_final_basis_requires_ocr_anchor": True,
            "traveler_final_disposition_uses_targeted_anchor_verification": True,
            "mutually_exclusive_multiple_or_ambiguous_selections_create_no_candidate": True,
            "machine_vision_requires_human_review": True,
        },
        "qdrant_entry_created": False,
    }
...
def render_review(output):
    ...
    testing = review.get("testing", {})
    final = review.get("final_result", {})
    hard = review.get("hardening", {})
    anchor = review.get("anchor_field_verification", {})
    corrected = sum(bool(r.get("semantic_correction")) for r in testing.get("candidates", []))
    ...
    lines = [
        ...
        "Serial: {}".format(identity.get("serial_number")),
        "",
        "ANCHOR-AWARE HARDENING SUMMARY",
        "------------------------------",
        "Testing candidates rejected before review: {}".format(hard.get("testing_rejection_count", 0)),
        "Final-result candidates rejected before review: {}".format(hard.get("final_rejection_count", 0)),
        "Non-test/setup/admin observations routed away: {}".format(hard.get("routed_observation_count", 0)),
        "Customer complaint/header allowed as TESTING_PERFORMED: NO",
        "Supporting final-result basis must exist in template OCR: YES",
        "Traveler final fields use printed-label anchor + local vision: YES",
        "Hard-coded absolute form coordinates used: NO",
        "Mutually-exclusive ambiguous group creates candidates: NO",
        "Globally ignored final-testing-duration field used as Final O.K. evidence: NO",
        "Globally ignored duration field masked before Final O.K. vision: YES",
        "Invalid numeric technician initials accepted: NO",
        "Checklist X/check marks semantically corrected to completed: {}".format(corrected),
        "",
        "ANCHOR FIELD VERIFICATION",
        "-------------------------",
        "Verifications: {}".format(anchor.get("verification_count", 0)),
        "Anchors found: {}".format(anchor.get("anchor_found_count", 0)),
        "Selected: {}".format(anchor.get("selected_count", 0)),
        "Not selected: {}".format(anchor.get("not_selected_count", 0)),
        "Ambiguous: {}".format(anchor.get("ambiguous_count", 0)),
        "Ambiguous mutually-exclusive groups: {}".format(anchor.get("ambiguity_group_count", 0)),
        "Anchor verification cache reused: {}".format(anchor.get("cache_reused_count", 0)),
        "",
    ]

    for record in anchor.get("verifications", []):
        v = record.get("verification", {}) or {}
        lines += [
            "FIELD {} [{}]".format(record.get("field_id"), record.get("verification_id")),
            "  Profile: {}".format(record.get("profile")),
            "  Target label: {}".format(record.get("target_label")),
            "  Anchor status: {}".format(record.get("anchor_status")),
            "  Anchor OCR match: {}".format((record.get("anchor_match") or {}).get("matched_text")),
            "  Anchor score: {}".format((record.get("anchor_match") or {}).get("score")),
            "  Selection: {}".format(v.get("selection_status")),
            "  Event mark: {}".format(v.get("event_mark")),
            "  Mark type: {}".format(v.get("mark_type")),
            "  Confidence: {}".format(v.get("confidence")),
            "  Cache: {}".format(record.get("cache_status")),
            "",
        ]

    for group in anchor.get("ambiguity_groups", []):
        lines += [
            "AMBIGUOUS GROUP {}".format(group.get("group")),
            "  Statuses: {}".format(group.get("all_field_statuses")),
...
def write_outputs(output, analyses, output_dir):
    ...
    write_json(output_dir / "testing_final_result_review.json", review)
    write_json(
        output_dir / "testing_final_result_page_analyses.json",
        {"fusion_version": VERSION, "analyses": analyses, "qdrant_entry_created": False},
    )
    write_json(
        output_dir / "testing_anchor_field_verifications.json",
        {
            "fusion_version": VERSION,
            "verification_count": anchor.get("verification_count", 0),
            "ambiguity_groups": anchor.get("ambiguity_groups", []),
            "verifications": anchor.get("verifications", []),
            "qdrant_entry_created": False,
        },
    )
    write_json(
        ...
        {
            "fusion_version": VERSION,
            "testing_rejections": hardening.get("testing_rejections", []),
            "final_rejections": hardening.get("final_rejections", []),
            "qdrant_entry_created": False,
        },
    )
    write_json(
        ...
        {
            "fusion_version": VERSION,
            "observations": hardening.get("routed_observations", []),
            "qdrant_entry_created": False,
        },
    )
    write_json(output_dir / "approved_repair_fields_with_testing_final.json", output)
    (output_dir / "testing_final_result_review.txt").write_text(
        render_review(output), encoding="utf-8"
    )
...
def main():
    ...
    parser.add_argument("--anchor-profiles", default=str(default_anchor_profiles_path()))
    ...
    try:
        ...
        anchor_profiles = load_anchor_profiles(args.anchor_profiles)
        ...
        field_verifications = analyze_anchor_fields(
            source_images,
            output_dir,
            model,
            timeout,
        ...
        decisions = load_decisions(output_dir)
        review = build_review(
            analyses, rules, decisions, source_data,
        ...
        if args.decision:
            ...
            decisions = load_decisions(output_dir)
            review = build_review(analyses, rules, decisions, source_data, field_verifications=field_verifications)
    ...
    anchor_summary = review.get("anchor_field_verification", {})
    print("Anchor field verifications:  {}".format(anchor_summary.get("verification_count", 0)))
    print("Anchor fields selected:      {}".format(anchor_summary.get("selected_count", 0)))
    print("Anchor fields ambiguous:     {}".format(anchor_summary.get("ambiguous_count", 0)))
    print("Anchor groups ambiguous:     {}".format(anchor_summary.get("ambiguity_group_count", 0)))
    print("Anchor cache reused:         {}".format(anchor_summary.get("cache_reused_count", 0)))
