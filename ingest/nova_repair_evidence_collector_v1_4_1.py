...
def collect_evidence(serial_folder, output_root, refs, traveler_output_root=None, inventory_only=False, hash_files=False, max_text_mb=25):
    ...
    for path in sorted(source.rglob('*')):
        ...
        if role_info['role'] == 'system_metadata':
            ...
            record = make_original_record(path, source, metadata_dir, inventory_only, hash_files, max_text_mb)
...
def main():
    ap = argparse.ArgumentParser(description='Nova DRL Repair Evidence Collector v1.4.1')
    ...
    out = Path(args.output_root).resolve() if args.output_root else Path.cwd()/'output'/'repair_evidence_collector_v1_4_1'/safe
