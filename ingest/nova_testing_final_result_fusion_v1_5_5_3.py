...
def default_rules_path():
    return (
        ...
        / "config"
        / "testing_final_result_rules_v1_5_5_3.json"
    )
...
def analyze_sources(
    ...
    analyses = []
    cache_dir = (
        Path(output_dir) / "page_analysis_cache_v1_5_5_3"
    )
    cache_dir.mkdir(parents=True, exist_ok=True)
...
# ============================================================================
# v1.5.5.3 ANCHOR-AWARE FORM ASSOCIATION OVERRIDES
# ============================================================================
...
def default_anchor_profiles_path():
    return (
        Path(__file__).resolve().parents[1]
        / "config"
        / "testing_anchor_profiles_v1_5_5_3.json"
    )
...
def _relative_anchor_crop(image_path, bbox, field, output_path):
    from PIL import Image
...
def _anchor_field_prompt(profile_name, field):
    ...
    ignore = field.get("ignore_nearby_values", []) or []
    special = ""
    if ignore:
        special += "\nIGNORE THESE NEARBY VALUES/FIELDS EVEN IF VISIBLE: {}.".format(
            "; ".join(ignore)
        )
    ...
    if profile_name == "DRL_TRAVELER_SHIPPING_FINAL_OK":
        special += (
            "\nThe nearby 48+ hours/final-testing-duration field is separate. A value such "
            "as 48+ or any hours value MUST NOT be used as the Final O.K. mark."
        )
...
7. event_mark must be the literal visible event-specific ink/mark associated
   with this target, not a schema word and not neighboring text.
8. Return JSON only.{special}
...
def _normalize_anchor_vision(parsed):
    parsed = parsed if isinstance(parsed, dict) else {}
    ...
    return {
        ...
        "event_mark": mark,
        "mark_type": mark_type,
        "technician_initials": parsed.get("technician_initials"),
...
def analyze_anchor_fields(
    ...
    results = []
    cache_dir = Path(output_dir) / "field_verification_cache_v1_5_5_3"
    cache_dir.mkdir(parents=True, exist_ok=True)
    model_available = ollama_model_exists(model) if not no_vision else False
        ...
        for field in fields:
            ...
            try:
                crop_info = _relative_anchor_crop(
                    source.get("image_path"), anchor_match["bbox"], field, crop_path
                )
            ...
            if response.get("status") == "ok":
                ...
                if parsed is None:
                    ...
                    base_record["verification"] = _normalize_anchor_vision(parsed)
            write_json(cache_path, base_record)
            results.append(base_record)
...
def build_testing_candidates_hardened_1553(analyses, rules, source_data):
    ...
    for analysis in analyses:
        ...
        for item in parsed.get("testing_items", []) or []:
            if _event_header_or_complaint_testing(
...
def render_review(output):
    ...
    lines = [
        ...
        "48+/hours value allowed as Final O.K. mark: NO",
