...
def default_rules_path():
    return (
        Path(__file__).resolve().parents[1]
        / "config"
        / "terminology_queue_rules_v1_5_2_2.json"
    )
...
def default_metadata_path():
    return (
        Path(__file__).resolve().parents[1]
        / "config"
        / "drl_metadata_identifiers_v1_5_2_2.json"
    )
...
def effective_glossary(base_glossary, decisions):
    ...
    result["generated_by"] = (
        "nova_terminology_review_queue_v1_5_2_2"
    )
...
def build_queue(
    ...
    suppression = suppression_summary(suppressions)
...
def main():
    ...
    parser.add_argument(
        ...
        default=str(
            Path.cwd()
            / "output"
            / "terminology_review_queue_v1_5_2_2"
        ),
    )
