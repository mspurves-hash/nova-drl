...
SCRIPT = ROOT / "analysis" / "nova_gb8_technician_answer_composer_v1_3_8_5.py"
...
def test_deterministic_fallback_preserves_group_ids():
    ...
    assert fb["findings"][0]["statement"] == "Historical GB8 pattern: Y Axis Drift."
    assert "serial" not in fb["findings"][0]["statement"].lower()
    assert "log" not in fb["findings"][0]["statement"].lower()
...
def test_render_displays_support_and_policy():
    ...
    assert "support: Y Axis Drift" in text
    assert "10 serials / 14 logs" not in text
    assert "38 distinct serials" not in text
    assert "rg_a" not in text
    assert "REPRESENTATIVE TRAVELER EVIDENCE" not in text
    assert "Traveler evidence and recurrence counts: hidden by default" in text
...
def test_support_text_never_exposes_internal_group_ids():
    ...
    assert txt == "Y Axis Drift"
    assert "rg_" not in txt


def test_display_sanitizer_removes_recurrence_counts():
    assert m.clean_display_statement("Drift recurs (10 serials / 14 logs).") == "Drift recurs"
    assert m.clean_display_statement("Seen across 10 distinct serials / 14 repair logs") == ""
    assert m.clean_display_statement("Check belt history across 10 serials / 14 logs.") == "Check belt history"


def test_counts_appear_only_when_evidence_expanded():
    result = {
        "query": "Y axis drifting",
        "hybrid": hybrid_result(),
        "composition": {
            "findings": [{"statement": "Y-axis drift recurs in 10 serials / 14 logs.", "support_group_ids": ["rg_a"]}],
            "suggested_checks": [],
            "caution": "",
            "rejected_items": 0,
        },
    ...
    # render_answer is downstream of validated composition in production, so sanitize explicitly here.
    result["composition"]["findings"][0]["statement"] = m.clean_display_statement(result["composition"]["findings"][0]["statement"])
    normal = m.render_answer(result)
    expanded = m.render_answer(result, show_evidence=True)
    assert "10 serials / 14 logs" not in normal
    assert "10 serials / 14 logs" in expanded
...
if __name__ == "__main__":
    ...
    for fn in tests:
        fn()
    print("PASS: Nova DRL GB8 Technician Answer Composer v1.3.8.5 tests")
