...
SCRIPT = ROOT / "analysis" / "nova_traveler_large_scale_reasoner_v1_3_7_1.py"
spec = importlib.util.spec_from_file_location("v1371", SCRIPT)
...
def test_metadata_filter_and_lanes():
    ...
    check(len(excluded) == 2, "metadata and unclear OCR should be preserved outside bulk reasoning set")
...
def test_fast_provisional_defaults_and_prompt():
    check(mod.DEFAULT_REASON_MODEL == "qwen25-drl:14b-q6-16k", "14B Q6 must be the v1.3.7.1 default reasoner")
    p = mod.STAGE1_PROMPT.lower()
    check("do not output singletons" in p, "fast stage-1 prompt must omit singleton output")
    check("python preserves them automatically" in p, "prompt must rely on Python singleton preservation")
    check("minor ocr" in p and "do not spend time" in p, "80/20 prompt must avoid forensic OCR hesitation")
    check("a1+a2 upper belts" in p, "prompt should encourage useful DRL-specific grouping")
    m = mod.MERGE_PROMPT.lower()
    check("80/20" in m and "practical repair/search concept" in m, "merge prompt must use fast provisional philosophy")


def test_stage1_omitted_candidates_are_preserved():
    rows = [
        cand("a", "130000001", "component_or_part", "A1 upper belt"),
        cand("b", "130000002", "component_or_part", "A2 upper belt"),
        cand("c", "130000003", "component_or_part", "odd one-off item"),
    ]
    for r in rows:
        r["reasoning_lane"] = "components"
    parsed = {"clusters": [{"concept_label": "A1+A2 upper belts", "concept_key": "a1_a2_upper_belts", "member_candidate_ids": ["a", "b"]}]}
    clusters, rejected = mod.validate_stage1_clusters(parsed, rows)
    check(not rejected, "clean 80/20 group should validate")
    check(any(set(x["member_candidate_ids"]) == {"a", "b"} for x in clusters), "useful model group should survive")
    check(any(set(x["member_candidate_ids"]) == {"c"} and x["stage1_origin"].startswith("fallback_singleton") for x in clusters), "omitted evidence must be preserved automatically")


def test_failure_counter_source_not_double_incremented():
    source = SCRIPT.read_text(encoding="utf-8")
    check("stage1_failures += 1\n                stage1_failures += 1" not in source, "stage-1 failure counter must increment once")
...
def main():
    ...
    test_fast_provisional_defaults_and_prompt()
    test_stage1_omitted_candidates_are_preserved()
    test_failure_counter_source_not_double_incremented()
    print("PASS: Nova DRL Large-Scale Batched Corpus Reasoner v1.3.7.1 tests")
