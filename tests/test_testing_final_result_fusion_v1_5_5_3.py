...
TARGET = ROOT / "ingest" / "nova_testing_final_result_fusion_v1_5_5_3.py"
spec = importlib.util.spec_from_file_location("tfr1553", str(TARGET))
mod = importlib.util.module_from_spec(spec)
spec.loader.exec_module(mod)

assert mod.VERSION == "1.5.5.3"
rules = mod.load_rules(ROOT / "config" / "testing_final_result_rules_v1_5_5_3.json")
anchors = mod.load_anchor_profiles(ROOT / "config" / "testing_anchor_profiles_v1_5_5_3.json")
...
assert any(r["reason"] == "shipping_final_ok_mark_is_testing_duration_not_final_ok_mark" for r in review_bad_ship["hardening"]["final_rejections"])

good_ship = [fv("final_ok","Final O.K.","Final O.K.","final_ok","selected","G 9/23/13",group=None,profile="DRL_TRAVELER_SHIPPING_FINAL_OK")]
...
print("PASS: Nova DRL Testing / Final Result Fusion v1.5.5.3 tests")
