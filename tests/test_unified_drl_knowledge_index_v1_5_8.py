...
UI = ROOT / 'tools' / 'nova_drl_unified_knowledge_index_v1_5_8.py'
...
ui = load('v158_ui_test', UI)

# HARD PROJECT POLICY: corpus volume owns product knowledge.
...
assert not hasattr(ui, 'load_standard_kit_rules')
...
with tempfile.TemporaryDirectory() as td:
    ...
    # 7800 family across 10 distinct events.  Only three rows have a populated
    # part_number field; the other seven are explicit parts_replaced evidence
    # whose replacement TEXT carries the 7800 identity.  This reproduces the
    # real MR-J2S-40A undercount that v1.5.8 fixes.
    explicit_pn = {1:'7800', 2:'HCPL7800', 3:'7800A'}
    text_only = {
        4:'Replaced HCPL-7800 and capacitors',
        5:'Changed HCPL7800A',
        6:'Shotgunned 630-HCPL-7800A-300E',
        7:'Replaced HPC-7800',
        8:'Installed HCL7800',
        9:'Replaced HCP1-7800',
        10:'Changed A7800',
    }
    for i,pn in explicit_pn.items():
        log=f'260101{i:03d}'
    ...
    for i,text in text_only.items():
        log=f'260101{i:03d}'
        parts.append({'repair_event_id':f'log_{log}','log_number':log,'equipment_family':events[i-1]['equipment_family'],
                      'part_number':None,'quantity':1,'text':text,'evidence_quote':text})
    ...
    # One-off OCR/noise stays searchable underneath but is suppressed from the 80/20 product list.
    parts.append({'repair_event_id':'log_260101012','log_number':'260101012','equipment_family':events[11]['equipment_family'],
                  'part_number':'RARE-X1','quantity':1,'text':'Replaced RARE-X1','evidence_quote':'Replaced RARE-X1'})
    ...
    assert ('7800',10) in got, got
    assert ('33uF',8) in got, got
    assert ('47uF',6) in got, got
    assert ('26C31',2) in got, got
    assert not any(pn == 'RARE-X1' for pn,_ in got), got
    assert all(n >= 2 for _,n in got), got
    ...
    assert 'REPAIR HISTORY' not in gd, gd.keys()
    assert 'TRACKING / PROJECT' not in gd, gd.keys()
    assert 'PROCUREMENT / REORDER' not in gd, gd.keys()
    assert 'SOURCE FILES' not in gd, gd.keys()
    ...
    assert 'TIMES REPLACED' in out
    assert 'REPORTED FAILURE' in out and 'TIMES SEEN' in out
    assert 'REPAIR HISTORY' not in out
    assert 'TRACKING / PROJECT' not in out
    assert 'RARE-X1' not in out

    blocks=ui.pdf_report_blocks('MR-J2S-40A',groups,1.0)
    txt='\n'.join(x[1] for x in blocks)
    assert 'REPORTED FAILURE' in txt and 'TIMES SEEN' in txt
    assert 'TIMES REPLACED' in txt
    assert '7800' in txt
    assert 'REPAIR HISTORY' not in txt
    assert 'TRACKING / PROJECT' not in txt
    assert 'RARE-X1' not in txt
    con.close()

print('PASS: Nova DRL Structured-Event Parts Aggregation v1.5.8 tests')
