...
SCRIPT = ROOT / "tools" / "nova_drl_unified_knowledge_index_v1_4_13.py"
spec = importlib.util.spec_from_file_location("v1413", SCRIPT)
...
def main():
    with tempfile.TemporaryDirectory() as raw:
        ...
        try:
            ...
            # v1.4.13 print cleanup: product identity is promoted, while
            # redundant/search-screen metadata is omitted from the PDF.
            assert b"EQUIPMENT / PRODUCT" in raw_pdf
            assert b"TRACKING / PROJECT" in raw_pdf
            assert b"PART OCCURRENCES" in raw_pdf
            assert b"INDEXED PARTS" not in raw_pdf
            assert b"Search time:" not in raw_pdf
            assert b"Coverage:" not in raw_pdf
            assert b"Customer PO" not in raw_pdf
            assert b"Indexed repair events:" not in raw_pdf
            ...
            assert '-N ""' not in it  # PowerShell 5.1 compatibility
            assert 'SilentlyContinue' in it and 'Test-NovaKey' in it
    ...
    print("PASS: Nova DRL Windows Engineer Client + Auto-Open Reports v1.4.13 tests")
