...
Purpose
-------
Run the completed v1.3.6.1 evidence-backed candidate ledger at practical corpus
speed using an 80/20 provisional-reasoning philosophy. Preserve the frozen
evidence layers, find useful recurring repair knowledge quickly, and leave
fine-grained corrections for later human review when a technician actually
questions a result.

Architecture
------------
1) v1.3.5.1 acquisition remains immutable evidence authority.
2) v1.3.6.1 prospecting remains the frozen evidence-backed candidate source.
3) Python removes only low-value identity/admin and unclear-OCR candidates from
   the bulk reasoning set while preserving them in the audit output.
4) Qwen2.5 14B Q6 performs fast provisional clustering in resumable batches.
5) Python validates IDs and preserves every eligible candidate exactly once;
   ungrouped or failed items fall back to singletons automatically.
6) Qwen2.5 14B Q6 performs concise hierarchical merge passes over compact
   summaries.
7) Python owns recurrence counts and provenance across distinct logs, source
   hashes, and robot serials/unit folders.

80/20 operating policy
----------------------
- Prefer useful recurring patterns over forensic perfection.
- Model labels may loosely normalize wording for organization/search.
- Minor OCR/shop-term wording differences do not require model hesitation.
- Ungrouped evidence is acceptable; Python preserves it automatically.
- Human amendments can correct provisional groupings later during real use.

Hard safeguards retained
------------------------
- No source image or raw transcription modification.
- No candidate raw-evidence modification.
- No automatic fact approval.
- No Qdrant writes.
- Candidate IDs and recurrence/provenance remain Python-controlled.
- Model labels/concept keys remain provisional organizational metadata only.
"""
...
DEFAULT_OUTPUT_ROOT = Path("/opt/nova-drl/output/traveler_large_scale_reason_v1_3_7_1")
...
STAGE1_PROMPT = r"""You are the FAST PROVISIONAL BATCH CLUSTERING layer for a DRL repair-history corpus.

Goal: find useful recurring repair concepts quickly. This is an 80/20 provisional pass, not a forensic review.
Each candidate has an immutable candidate ID plus exact source evidence already preserved elsewhere.

Return JSON only in this exact shape:
{
  "clusters": [
    {
      "concept_label": "short useful provisional label",
...
RULES:
- Output only useful groups containing 2 or more supplied candidate IDs. Do NOT output singletons.
- It is fine to omit candidates that do not obviously belong in a useful group; Python preserves them automatically.
- Use each candidate ID at most once and never invent IDs.
- Group by practical DRL meaning. Prefer specific axis/subassembly/component/action labels when obvious
  (for example A1+A2 upper belts) instead of collapsing everything to a generic noun such as belt or motor.
- Broad practical grouping is acceptable when the evidence is close enough to help a technician search prior repairs.
- Do not spend time resolving minor OCR, spelling, punctuation, abbreviation, or shop-term wording differences.
- Do not explain your choices, quote evidence, or add commentary.
- Keep labels and keys concise. They are provisional search/organization metadata, not approved facts.
"""

MERGE_PROMPT = r"""You are the FAST PROVISIONAL HIERARCHICAL MERGE layer for DRL repair-history clusters.

Goal: combine obviously equivalent or practically interchangeable provisional clusters so recurring repair patterns
become useful to technicians. This is an 80/20 organizational pass; exact evidence remains preserved separately.
...
RULES:
- Output only merge groups containing 2 or more supplied cluster IDs.
- Merge clusters when they represent the same practical repair/search concept or are close enough to be useful together.
- Prefer useful axis/subassembly/component/action specificity when it is obvious, but do not overthink minor wording differences.
- Minor OCR, spelling, punctuation, abbreviation, and shop-term variants are not reasons to avoid an otherwise useful merge.
- Use each cluster ID at most once in your response and never invent IDs.
- If no useful merge is apparent, return {"merge_groups":[]}.
- Return JSON only. No explanations, evidence quotes, markdown, or commentary.
- Labels and keys are provisional organizational metadata, not approved facts.
"""
...
def render_summary(
    ...
    lines += [
        ...
        "v1.3.6.1 raw evidence modified/replaced: NO",
        ...
        "Operating philosophy: FAST PROVISIONAL 80/20; amend later when needed",
        "Automatic human approval: NO",
        "Qdrant writes: OFF",
        "",
        "Full provenance: recurring_patterns_v1_3_7_1.json",
        "All final clusters: final_cluster_ledger_v1_3_7_1.json",
        "Excluded-but-preserved metadata/unclear candidates: reasoning_exclusion_audit_v1_3_7_1.json",
    ]
    (output_root / "large_scale_reasoning_summary_v1_3_7_1.txt").write_text("\n".join(lines) + "\n", encoding="utf-8")


def main() -> int:
    parser = argparse.ArgumentParser(description=f"Nova DRL Fast Provisional Corpus Reasoner v{VERSION}")
    ...
    parser.add_argument("--stage1-num-predict", type=int, default=2048)
    parser.add_argument("--merge-num-predict", type=int, default=2048)
    ...
    parser.add_argument("--json-retries", type=int, default=1, help="One retry after malformed JSON; then fall back safely and keep moving")
    parser.add_argument("--timeout", type=int, default=1800)
    parser.add_argument("--plan-only", action="store_true", help="Build/print the large-scale batching plan without reasoning-model calls")
    ...
    save_json(output_root / "reasoning_exclusion_audit_v1_3_7_1.json", {
    ...
    plan = {
        ...
        "reason_model_requested": args.reason_model,
        "num_ctx": args.num_ctx,
        "stage1_num_predict": args.stage1_num_predict,
        "merge_num_predict": args.merge_num_predict,
        "json_retries_after_first_attempt": args.json_retries,
    ...
    save_json(output_root / "large_scale_reasoning_plan_v1_3_7_1.json", plan)

    print(f"# Nova DRL Fast Provisional Corpus Reasoner v{VERSION}")
    ...
    print(f"Reasoning eligible:  {len(eligible)}")
    ...
    if args.plan_only:
        ...
        print(f"Plan: {output_root / 'large_scale_reasoning_plan_v1_3_7_1.json'}")
        return 0
    ...
    save_json(output_root / "stage1_cluster_ledger_v1_3_7_1.json", {
        "reasoner_version": VERSION,
        "raw_stage1_cluster_count": len(all_stage1_clusters),
        "after_exact_model_key_consolidation_count": len(initial_clusters),
        "clusters": initial_clusters,
    })
    save_json(output_root / "stage1_rejected_model_groups_v1_3_7_1.json", all_stage1_rejections)
    save_json(output_root / "stage1_run_summary_v1_3_7_1.json", stage1_run_meta)
    ...
    for round_num in range(1, max(0, int(args.max_merge_rounds)) + 1):
        ...
        save_json(output_root / f"merge_round_{round_num:02d}_stats_v1_3_7_1.json", stat)
    ...
    save_json(output_root / "final_cluster_ledger_v1_3_7_1.json", {
        "reasoner_version": VERSION,
    ...
    save_json(output_root / "recurring_patterns_v1_3_7_1.json", {
        "reasoner_version": VERSION,
    ...
    save_json(output_root / "merge_rejected_model_groups_v1_3_7_1.json", all_merge_rejections)
    ...
    save_json(output_root / "reasoner_manifest_v1_3_7_1.json", reasoner_manifest)
    ...
    print(f"Summary: {output_root / 'large_scale_reasoning_summary_v1_3_7_1.txt'}")
    print(f"Patterns: {output_root / 'recurring_patterns_v1_3_7_1.json'}")
    print(f"Manifest: {output_root / 'reasoner_manifest_v1_3_7_1.json'}")
    return 0
