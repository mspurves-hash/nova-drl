#!/usr/bin/env python3
"""
Nova DRL Full Corpus Ingester v1.5.1
...
supporting source-path sets exactly match, v1.5.1 reuses that event instead of re-reading
...
DEFAULT_OUTPUT_ROOT = Path("/opt/nova-drl/output/drl_full_corpus_v1_5_1")
...
def load_base():
    ...
    # Patch proven base helpers to v1.5.1 policy/cache identity.
    mod.VERSION = VERSION
...
def extract_new_events(args: argparse.Namespace, events: Sequence[Dict[str, Any]], vision_records: Sequence[Dict[str, Any]], reuse: Dict[str, Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for i, event in enumerate(events, 1):
        ...
        row = {
            ...
            "ingest_source": "v1.5.1_full_corpus",
        }
        out.append(row)
    ...
    write_jsonl(Path(args.output_root) / "repair_events_v1_5_1.jsonl", out)
    return out
...
def write_outputs(args: argparse.Namespace, corpus: Dict[str, Any], selection: Dict[str, Any], events: Sequence[Dict[str, Any]], event_rows: Sequence[Dict[str, Any]], vision_count: int, reused_events: int) -> None:
    ...
    base.save_json(out_root / "source_selection_v1_5_1.json", {
        "version": VERSION,
    ...
    base.save_json(out_root / "repair_event_plan_v1_5_1.json", {
        "version": VERSION,
    ...
    write_jsonl(out_root / "replacement_mentions_v1_5_1.jsonl", parts_rows)
    write_jsonl(out_root / "rma_refs_v1_5_1.jsonl", rma_rows)
    write_jsonl(out_root / "customer_po_refs_v1_5_1.jsonl", po_rows)
    write_jsonl(out_root / "procurement_refs_v1_5_1.jsonl", order_rows)
    ...
    write_csv("replacement_mentions_v1_5_1.csv", parts_rows, ["repair_event_id","log_number","equipment_family","part_number","quantity","text","evidence_quote"])
    write_csv("rma_lookup_v1_5_1.csv", rma_rows, ["repair_event_id","log_number","equipment_family","rma_number","evidence_quote","source_path"])
    write_csv("customer_po_lookup_v1_5_1.csv", po_rows, ["repair_event_id","log_number","equipment_family","customer_po","evidence_quote"])
    write_csv("procurement_refs_v1_5_1.csv", order_rows, ["repair_event_id","log_number","equipment_family","supplier","order_ref","description","manufacturer_pn","quantity","evidence_quote","source_path"])
    ...
    lines = [
        ...
        f"Vision records actually read this run: {vision_count}",
    ...
    (out_root / "drl_full_corpus_summary_v1_5_1.txt").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    base.save_json(Path(args.output_root) / "full_corpus_manifest_snapshot_v1_5_1.json", corpus)
    base.save_json(Path(args.output_root) / "source_selection_v1_5_1.json", selection)
    base.save_json(Path(args.output_root) / "repair_event_plan_v1_5_1.json", {"version": VERSION, "events": events})
    ...
    vision_rows = base.acquire_evidence(args, source_records)
    base.save_json(Path(args.output_root) / "vision_source_manifest_v1_5_1.json", {"version": VERSION, "records": vision_rows})
    if args.vision_only:
    ...
    print("Accepted facts:                  0")
    print("Qdrant:                          OFF")
    print(f"Summary: {Path(args.output_root) / 'drl_full_corpus_summary_v1_5_1.txt'}")
    print(f"Events:  {Path(args.output_root) / 'repair_events_v1_5_1.jsonl'}")
    print(f"Parts:   {Path(args.output_root) / 'replacement_mentions_v1_5_1.jsonl'}")
    print(f"RMA:     {Path(args.output_root) / 'rma_lookup_v1_5_1.csv'}")
    print(f"Orders:  {Path(args.output_root) / 'procurement_refs_v1_5_1.csv'}")
    return 0
