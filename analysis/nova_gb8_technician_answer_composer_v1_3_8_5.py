#!/usr/bin/env python3
"""Nova DRL GB8 Technician Answer Composer v1.3.8.5.
...
DEFAULT_CONFIG_NAME = "gb8_answer_composer_policy_v1_3_8_5.json"
...
- Do NOT include serial counts, repair-log counts, recurrence counts, or internal group IDs in findings, suggested checks, or caution text. Those are evidence/provenance only.
...
def clean_display_statement(value: Any, limit: int = 520) -> str:
    """Remove provenance-style recurrence counts from technician-facing prose.

    Counts remain in frozen JSON and opt-in evidence display; normal answers stay clean.
    """
    text = compact(value, limit)
    patterns = [
        r"\b(?:seen\s+(?:across\s+)?|observed\s+(?:across\s+)?|covering\s+|across\s+)(?:approximately\s+)?\d+\s+(?:distinct\s+)?serials?\s*(?:and|/|,)?\s*\d+\s+(?:repair\s+)?logs?\b",
        r"\s*\(\s*\d+\s+(?:distinct\s+)?serials?\s*/\s*\d+\s+(?:repair\s+)?logs?\s*\)",
        r"\s*\[\s*\d+\s+(?:distinct\s+)?serials?\s*/\s*\d+\s+(?:repair\s+)?logs?\s*\]",
        r"\b\d+\s+(?:distinct\s+)?serials?\s*/\s*\d+\s+(?:repair\s+)?logs?\b",
    ]
    for pat in patterns:
...
def _validated_items(items: Any, allowed_ids: set[str], limit: int) -> Tuple[List[Dict[str, Any]], int]:
    ...
    for item in items:
        if not isinstance(item, dict):
            rejected += 1
            continue
        statement = clean_display_statement(item.get("statement"), 520)
        ids = []
...
def validate_composition(parsed: Any, allowed_ids: set[str], max_findings: int, max_checks: int) -> Dict[str, Any]:
    ...
    caution = clean_display_statement(parsed.get("caution"), 420)
    return {
        "findings": findings,
...
def deterministic_fallback(hybrid_result: Mapping[str, Any], max_findings: int) -> Dict[str, Any]:
    ...
    for row in (hybrid_result.get("results") or [])[: max(1, int(max_findings))]:
        ...
        if not gid:
            continue
        findings.append({
            "statement": f"Historical GB8 pattern: {label}.",
...
def render_answer(result: Mapping[str, Any], show_retrieval: bool = False, show_evidence: bool = False, hybrid_renderer: Any | None = None) -> str:
    ...
    lines = [
        ...
        "ANSWER",
        "------",
        f"Hybrid retrieval selected {len(rows)} recurring GB8 patterns for this question.",
    ]
    if labels:
    ...
    if show_evidence:
        ...
        lines += ["", "Traveler evidence and recurrence counts: hidden by default; use --show-evidence (or :evidence on) when needed."]
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Nova DRL GB8 Technician Answer Composer v1.3.8.5 — clean technician output")
    ...
    parser.add_argument("--config", default="", help="v1.3.8.5 answer composer policy JSON")
