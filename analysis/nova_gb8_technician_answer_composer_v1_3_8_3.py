...
DEFAULT_CONFIG_NAME = "gb8_answer_composer_policy_v1_3_8_3.json"
...
def support_text(ids: Sequence[str], by_id: Mapping[str, Mapping[str, Any]]) -> str:
    ...
    for gid in ids:
        ...
        label = compact(g.get("concept_label") or g.get("concept_key") or gid, 90)
        serials = int(g.get("distinct_serial_count") or 0)
        logs = int(g.get("distinct_log_count") or 0)
        parts.append(f"{label} [{gid}; {serials} serials/{logs} logs]")
    return "; ".join(parts)


def render_answer(result: Mapping[str, Any], show_retrieval: bool = False, hybrid_renderer: Any | None = None) -> str:
    ...
    for i, row in enumerate(rows[:5], 1):
        g = row.get("group") or {}
        gid = str(row.get("group_id") or group_id(g))
        label = compact(g.get("concept_label") or g.get("concept_key") or gid, 120)
        lines.append(f"{i}. {label} | {int(g.get('distinct_serial_count') or 0)} serials / {int(g.get('distinct_log_count') or 0)} logs | {gid}")
        for ex in (row.get("examples") or [])[:2]:
            lines.append(f"   - {ex.get('log_number') or '?'} | {ex.get('serial_number') or '?'} | {compact(ex.get('raw_source_text'), 260)}")
...
def interactive_loop(hmod: Any, qmod: Any, pmod: Any, index: Mapping[str, Any], aliases: Mapping[str, List[str]], args: argparse.Namespace, hybrid_policy: Mapping[str, Any], composer_policy: Mapping[str, Any], api_key: str) -> int:
    ...
    print("Type only the technician question. Commands: :status, :retrieval on, :retrieval off, :quit")
    ...
    while True:
        ...
        print(render_answer(result, show_retrieval=show_retrieval, hybrid_renderer=hmod.render_hybrid))


def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Nova DRL GB8 Technician Answer Composer v1.3.8.3")
    ...
    parser.add_argument("--config", default="", help="v1.3.8.3 answer composer policy JSON")
    ...
    if args.json:
        ...
        print(render_answer(result, show_retrieval=args.show_retrieval, hybrid_renderer=hmod.render_hybrid), end="")
    return 0
